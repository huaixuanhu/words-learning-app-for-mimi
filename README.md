...
- V2-8-2.3 使用 `PF-001` 起的连续编号集中处理 Preview 反馈。PF-001 为 `High / Closed`，使用独立 TTS Cache、Kill Switch、全局额度和 additive `0005`，不复用 Gemini ledger；用户与 Mimi 均确认音效可接受，Mimi 的设备环境和 iPhone + Safari 兼容性仍未形成证据。PF-002 为 `Normal / Preview-ready`：桌面入口改为 `Review / New Learning`，手机入口为 `Learn`；Home/Study 使用两项目标层级；Session 文案更明确；Instrument Serif 只用于英文显示标题与大词。Exact protected Preview 的 1280/390 px、面板等高、字体范围、保护状态和运行日志机器检查均通过，仍等人工复测。该阶段仍不包含 SSO。
