...
Stage 7 mobile-first vocabulary app for Mimi, with local vocabulary, single input, batch JSON import, recognition-only review scheduler, flashcards, local export / backup, local multi-person adapter, durable storage readiness, development / preview Neon bootstrap, server-only runtime Postgres adapter, controlled Preview smoke checks, backup import harness, development / preview UI runtime cutover, read-only Preview UI runtime verification, and a locally accepted darker sage visual design pass with Motion for React interaction animation plus Stage 7.2 Mimi brand refinement, Stage 7.3 ChillRoundF font trial, Stage 7.4 light / dark theme toggle, Stage 7.5 soft click sound audition, Stage 7.6 UI-only sound settings, Stage 7.7 final acceptance, Stage 7.8 dual-track UI refinement, and Stage 7.9 Recognition / Active data import refinement.
...
- Local single vocabulary input, library search/edit/archive/restore, and `.json` / pasted JSON import preview.
- Local Recognition Vocabulary（阅读词汇）review sessions with four fixed ratings, review event/state updates, and customizable Recognition daily limit（每日上限）.
- Local Active Vocabulary（输出词汇）classification and lower-volume daily limit setting; dictation, spelling, writing practice, and AI API（人工智能接口）feedback remain future work.
