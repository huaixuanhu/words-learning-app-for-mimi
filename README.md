...
Stage 8 Review Memory Algorithm implementation for Mimi's mobile-first vocabulary app, with local vocabulary, single input, batch JSON import, recognition-only review scheduler, flashcards, local export / backup, local multi-person adapter, durable storage readiness, development / preview Neon bootstrap, server-only runtime Postgres adapter, controlled Preview smoke checks, backup import harness, development / preview UI runtime cutover, read-only Preview UI runtime verification, a locally accepted darker sage visual design pass with Motion for React interaction animation plus Stage 7.2 Mimi brand refinement, Stage 7.3 ChillRoundF font trial, Stage 7.4 light / dark theme toggle, Stage 7.5 soft click sound audition, Stage 7.6 UI-only sound settings, Stage 7.7 final acceptance, Stage 7.8 dual-track UI refinement, Stage 7.9 Recognition / Active data import refinement, Stage 7.10 Library / Review controls, Stage 7.11 Review rollback / auto-refresh controls, Stage 8-B / 8-C / 8-D / 8-E / 8-F Recognition Vocabulary（阅读词汇）memory behavior, backup compatibility, and Postgres Production handoff, Stage 6B-P1 Postgres Production runtime planning, and a Stage 8 gate that blocks Production until the Recognition memory algorithm is accepted.
...
- Stage 8-F Postgres Production handoff: Stage 6B-P1 now records the final V1 Recognition FSRS state shape, keeps `dueAt` exact with natural-day queue semantics in app code, and keeps the real `0002` Production migration deferred until explicit Stage 6B-P1 implementation approval.
...
- Stage 6B Production execution plan: `plan_docs/PLAN_V1_STAGE6B_PRODUCTION_EXECUTION.md` records the 2026-07-08 user decision to launch formal V1 with shared Postgres Production. `plan_docs/PLAN_V1_STAGE6B_P1_POSTGRES_PRODUCTION_RUNTIME.md` is the required runtime bridge before merge, Production migration/import, and deployment, but Stage 8 now blocks that implementation until the review memory state shape is accepted.
- Stage 8 Review Memory Algorithm: `plan_docs/PLAN_V1_STAGE8_REVIEW_MEMORY_ALGORITHM.md` documents the Recognition-only FSRS-6 plan, same-session repeat for failed Recognition ratings, Active Vocabulary no-scheduling boundary, neutral `difficulty` / `stability` naming, and V2-compatible separate future Active scheduler dimensions such as `review_profile`, `skill_type`, or `activity_type`.
