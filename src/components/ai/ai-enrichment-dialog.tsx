...
export function AiEnrichmentDialog({
  ...
  const [cacheStatus, setCacheStatus] = useState<"generated" | "cached" | null>(null);
  ...
  const notice = LOCAL_FIXTURE_LINEAGE.notice;

  const createPreview = async () => {
    if (!item || !localPreviewEnabled || isWorking) return;
    setIsWorking(true);
    setMessage("");
    ...
    try {
      ...
      setDraftId(staged.draft.id);
      const visibleDraft = staged.draft.status === "accepted" && staged.draft.acceptedContent
      ...
      setDecision(staged.draft.status);
      setCacheStatus(staged.cacheStatus === "cached" || fixture.cacheStatus === "cached"
        ? "cached"
        : "generated");
      setMessage(
        staged.draft.status === "accepted"
          ? "This preview was already accepted."
          : "Preview ready. Edit anything before accepting.",
      );
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Could not create the local preview.");
    } finally {
      setIsWorking(false);
    }
  };
    ...
    try {
      ...
      setMessage(
        nextDecision === "accepted"
          ? "Accepted. You can now add a candidate to learning."
  ...
  const description = item
    ? `Local enrichment preview for ${item.surfaceText}`
    : "Local enrichment preview";
  const closeDialog = () => {
    setDraftId(null);
    setDraft(emptyDraft());
    setDecision("draft");
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p id={descriptionId} className="mt-1 text-sm text-[var(--mimi-text-soft)]">
            {description}
          </p>
        </div>
        <button
      ...
      <div className="mt-4 rounded-md border border-[var(--mimi-border)] bg-[var(--mimi-surface-muted)] p-3">
        ...
        <p className="mt-1 text-xs leading-5 text-[var(--mimi-text-soft)]">
          This fixed preview checks the V2 flow. Real Gemini generation stays off in Stage 7A.
        </p>
        <details className="mt-2 text-xs leading-5 text-[var(--mimi-text-soft)]">
          ...
          <p className="mt-1">
            If enabled later, only {AI_DISCLOSURE.sentFields.join(", ")} would be sent.
            Identity, notes, study history, typed answers, and audio stay out. You will confirm
            the current disclosure before the first real request.
          </p>
        </details>
      </div>
      ...
      {!draftId ? (
        <PressableButton
          type="button"
          disabled={!item || !localPreviewEnabled || isWorking}
        ...
        >
          ...
          {isWorking ? "Preparing..." : "Create local preview"}
        </PressableButton>
      ) : (
        <div className="mt-5 grid gap-5">
          <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--mimi-text-muted)]">
            <span className="mimi-pill px-2 py-1 font-semibold">
              {cacheStatus === "cached" ? "Cached preview" : "Fresh preview"}
            </span>
            <span className="mimi-pill px-2 py-1 font-semibold">
          ...
          <section className="grid gap-3">
            ...
            {draft.similarWords.length ? draft.similarWords.map((candidate, index) => (
              ...
              <p className="text-sm text-[var(--mimi-text-muted)]">No similar word in this fixed preview.</p>
            )}
          </section>
          ...
          <section className="grid gap-3">
            ...
            {draft.confusableWords.length ? draft.confusableWords.map((candidate, index) => (
              ...
              <p className="text-sm text-[var(--mimi-text-muted)]">No confusable word in this fixed preview.</p>
            )}
          </section>
