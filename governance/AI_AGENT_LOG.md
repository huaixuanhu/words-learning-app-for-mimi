...
- First real monthly backup on committed `2a334a0` failed in 34 seconds and retained no verified archive. Preserved its original failed receipt. A credential-free local PostgreSQL probe reproduced failed startup at a 127-byte socket path and successful startup at 52 bytes; stopped/cleaned its cluster. Fixed the private temporary path and safe error reporting, with a single explicit reviewed-retry receipt that cannot reset the original attempt or enable scheduled retries. Forty focused tests and targeted lint passed; original failure phase remains unknown because its stderr was discarded.
- Completed the one reviewed real retry on committed `2a1caa3`: 1,498,231-byte age archive, 0600 permissions, isolated PostgreSQL 17 restore parity, original failure receipt hash unchanged, lock released and no old archive removed. The exact patched 82-byte socket layout also passed independent local startup/cleanup. Updated existing monthly heartbeat `mimi` to ACTIVE after this evidence; ordinary scheduled execution explicitly excludes the reviewed-retry flag. Updated the AGENTS backup-policy pointer to the latest user-authorized lifecycle override; no wider authority changed.
...
- Committed: initial V2.3 candidate `0f242d5`. The first staged diff check caught a trailing blank line in a newly added TTS file; it was removed before the successful commit. The earlier automatic-review rejection was not bypassed; the user supplied the requested authorization.
- Inspected: reused source-pinned Neon target-selection helpers with existing Keychain values in process memory. Two bounded probes reached the intended target but the initial READ ONLY transaction failed with SQLSTATE `53000`; driver-message classification confirmed network-transfer-quota exhaustion. No learner-row or count query executed. Control-plane GETs confirmed Free_v3, September transfer 6,330,077,483 bytes, unchanged Production identity and a ready main branch with 44,376,064 logical bytes. Separate operations metadata was read without querying data again.
- Changed files: Postgres client and tests for transaction client error handling, unsafe-connection discard, finite connection acquisition and existing Vercel pool helper; vocabulary data provider and tests for malformed-response recovery and newer cross-tab selection; Architecture, V2 master/child plan, Changelog and this log.
- Evidence ceiling: record counts and full data completeness remain blocked by Neon quota. Normal per-card full snapshots and extra-tab reads are confirmed amplification, but actual 6.33 GB is not attributed to individual routes. Synthetic loop tests stabilized; synthetic byte modeling and retained backup aggregate byte counts are supporting scale evidence only. No private learner content was inspected for that model.
- Risk disposition: connected-transaction failure and loading-boundary findings were fixed locally. Per-card read reduction, local corrupt-store preservation, uncertain-write reconciliation and browser backup consistency remain explicit future work in the V2.3 plan.
- Validation: final full Vitest passed 102 files / 673 tests, existing remote integration file/test skipped; typecheck and credential-free `next build --webpack` passed. ESLint initially reported one unused variable in the ignored temporary diagnostic script; removed that binding without rerunning remote operations. Backup/UI evidence from the initial candidate remains applicable to their unchanged contracts. Governance and final diff inventory were checked before the follow-up local commit.
- Safety notes: ignored evidence under `local_artifacts/v2-3-diagnosis` contains only metadata, identity hashes, aggregate usage and safe error categories. Credentials/connection strings stayed in memory. No Production data write, migration, restore, backup dump, provider call, paid-plan change, push or deployment occurred. Query attempts stopped after quota classification; no branch or transport workaround was used to evade the quota.

## 2026-09-13 AEST — initial local repair before follow-up approval

- Task and agreement: user requested a local V2.3 branch and repairs for Library disappearance, overnight learning-day attribution and Home Insight density. The user selected 06:00 and retained grouped date/recall charts. Scope and lineage: `plan_docs/PLAN_V2_3_STORAGE_STUDY_DAY_HOME.md`; affected/target/working authority remains Tier 3.
- Inspected: clean `main`, current source/plans and the existing authenticated application/Vercel pages. Production remains `dpl_HXmxx55SPvmhAwhFwaV6eRmTtySg` / `3a64aa2`. Library showed zero total without filters. Logs confirmed storage GET 500, health 503 and uncaught idle-Pool errors with process exit 129. The Vercel connector returned permission-denied, so existing browser access supplied the log evidence; no permission setting was changed.
- Changed: client loading/recovery boundary and tests; sanitized Pool error handling and tests; dedicated 06:00 day/window transition and shared rollover/wake logic; new/reviewed rhythm counts, preserved forecast groups and trusted-clock updates; canonical routing, plan, architecture and change records. Moved the existing TTS service resolver verbatim to its adjacent module to resolve a pre-existing invalid route export found by the final production build; provider/cache/accounting and POST logic were mechanically compared with HEAD and remain unchanged.
- Preserved: cloud learner selection, accepted data on refresh failure, immutable plan/event history, old backup/schema/FSRS contracts, separate Track histories, server-owned expired-token checks, transactional reset confirmations, calendar-based AI/TTS limits, credential/provider/Production/GitHub approval boundaries. No automatic replay of uncertain writes.
- Validation: full Vitest passed 102 files / 661 tests, with the existing Postgres integration file/test skipped; lint, typecheck and three fixture backup checks passed. Backup v4 / Schema 6 roundtrip preserved legacy, transition and normal successor plan evidence. Independent review identified and fixed invalid clocks after sleep, timezone transition gaps and stale Home error text after recovery. Browser evidence used a credential-free temporary source export: real local Home empty/loading state plus a temporary visual host with 42 synthetic words / 97 events for Insights, track/range switching, error layout and keyboard Retry. 1280 px and 320 px layouts were inspected; mobile content width equaled viewport width. Visual-host Retry proves control interaction; real data-read failure/retry is covered by provider transition tests.
- Validation environment: Chrome local-file upload permission prevented fixture file selection. No browser permission was changed; synthetic data was supplied only inside the temporary local visual host. Watchpack reached the local watcher limit; the temporary dev server used polling, without repository or system setting changes. The credential-free source export passed `npm run build -- --webpack` after removing the temporary visual route. Both temporary dev servers stopped, temporary browser tabs closed and the viewport override reset. Final governance evidence is recorded in the V2.3 plan.
- Safety/result ceiling: no `.env`/credential value, Production data write/restore/migration, new provider request, remote push or deployment. Local fixes do not prove that the underlying Neon connection issue is resolved or that Production records are complete. The next consequential action requires explicit Production inspection/release approval.
- Commit boundary: automatic approval review rejected the explicit-path `git add` plus local `git commit` command, interpreting the current branch/fix request as lacking separate commit authority. The command did not execute; no retry or workaround followed. The validated changes remain uncommitted on `codex/v2.3`, pending user confirmation.

## 2026-09-04 00:19 AEST

- Task: complete the user-authorized V2.2 Staging rehearsal, Production recovery/migration, Git/Vercel activation, runtime verification and documentation closeout.
- Plan agreed: yes. The user confirmed the combined V2.2 change, confirmed `0007`, then explicitly instructed Codex to continue until Production release under `plan_docs/PLAN_V2_2_PRODUCTION_RELEASE.md`.
- Current affected authority: Tier 3 Production database constraints, external encrypted backups, GitHub `v2.2`/`main`, Vercel Production and one bounded TTS acceptance request. No Non-Scope action was authorized.
- Candidate and local evidence: exact candidate `727a7089acbaa51fb00820dea7d39fc7aaa543d1` passed focused product coverage 12 files / 80 tests, final release-guard coverage 4 files / 58 tests across the candidate runs, full Vitest 98 files / 607 tests with the existing Postgres integration file/test skipped, ESLint, TypeScript, three backup dry-runs, Production build, Tier 3 governance, diff checks and full/Production npm audits with zero findings.
- Staging result: the exact retained branch safely unarchived through its existing endpoint. Pinned `0007` completed once; V2 constraint SHA-256 is `69bedcec75fba6e764630713c134b60d9c174e8432bc27e27b3128aa128db615`; all checked invariants are zero; and the complete 22-table digest stayed `772d189c78a7de0212b2821b076bf96c8a042300ac4e57d43bf4af3d30d10402`. Ignored evidence SHA-256 is `5449df4eb81836466128d75fc83cd5d1227ee1a00145bb466f23aafa27582291`.
- Pre-change recovery: external encrypted archive SHA-256 `193b22f0ff9bb0c8a178326bd1addcbca641348a4578fd3c9f47cf9d757c86fb`, evidence SHA-256 `4004a94ca8ed8ea6255b8427b2e6a4b793e439631c19bb9f726802d464edbfd0`, permission `0600`, and isolated PostgreSQL 17 restore equality passed.
- Production migration result: pinned `0007` completed once. Schema stayed Version 6; V2 constraint SHA matches Staging; 282 vocabulary items, 125 review states and 441 review events remained unchanged; every checked invariant is zero; and the complete 22-table migration digest stayed `a6889675ba463be6c1faabb119b850910f1442a18bd20ce58c0eb76ef4692261`. Ignored evidence SHA-256 is `2a4c545ec534a880d110a613f28bc03383bf795a52d65261f6f10add35cc9b6d`.
- Activation result: pushed candidate `727a7089acbaa51fb00820dea7d39fc7aaa543d1` non-force to `v2.2`, fast-forwarded and pushed `main`, and verified GitHub's successful deployment status. Vercel deployment `dpl_CzhB6hqAxeXXpYHRpcB9a2GHPyHW` is Ready, Current and Production, sourced from the exact candidate; the canonical domain remains unchanged and the application plus `/api/tts` are in `syd1`.
- Runtime result: anonymous canonical-domain access returned Basic Auth `401`; authenticated Home, Settings and Library loaded; Home and Library reported 282 entries; Settings showed `Cloud voice` and `Use device voice` with Cloud selected; `/api/storage/health` returned `200`; browser console warning/error lists were empty; and the 30-minute Vercel scan showed zero Warning/Error/Fatal entries and no HTTP 5xx. The build completed successfully; only dependency lifecycle/support notices were present.
- TTS evidence boundary: no fresh `/api/tts` request is claimed. The available authenticated Chrome page could not send the exact permitted `review` POST without credential access, and the Settings preview would send different text. No `.env` value, Keychain Basic Auth value, login configuration or provider control was read or changed. `/api/tts` is deployed among the 22 `syd1` Functions, local Cloud/device routing tests pass, and Production accounting stayed at one historical successful run with zero active work.
- Post-release recovery: external encrypted archive SHA-256 `d64a59b7de9d5baac64b5a65347f2026f9fc474720471b1d90c057e4ce807dee`, evidence SHA-256 `a4b01fc9d4c5fd11843f96eecaa8fd277af3cde1335f2e189ca9d01df7d7db08`, permission `0600`, and isolated PostgreSQL 17 restore equality passed.
- Runtime side effect: after the authenticated page check crossed local midnight, only `daily_study_plans` changed from 14 to 16 rows. All vocabulary, review-state, review-event, settings, AI and TTS table digests stayed unchanged. The timing and one `/api/study` request support normal daily-plan materialization as the likely cause; it remains an inference. No cleanup, restore or compensating write was attempted.
- Changed files: V2.2 product/release plans, V2 master, Architecture, README, AGENTS, Changelog and this log. This closeout changes documentation only; the released application, dependency lockfile and migration bytes remain candidate `727a7089acbaa51fb00820dea7d39fc7aaa543d1`.
- Validation: the released candidate's full gate remains the exact evidence listed above. For this documentation-only closeout, `npm run governance:check`, `git diff --check`, exact file inventory and final branch/deployment identity are required before handoff; no application test result is reused as evidence for changed runtime bytes because no runtime byte changed.
- Safety notes: no raw credential, connection URI, learner text, person id, provider body or backup identity entered Git/output. No Gemini request, credential/provider/quota/domain/Firewall/Vercel-setting change, learner-data cleanup, restore, force push, history rewrite, tag or deletion occurred. The exact final documentation commit and Git-triggered deployment are verified after commit/push and stored in ignored closeout evidence plus the human handoff because a commit cannot contain its own final hash.
- Reason: finish the accepted V2.2 release with exact migration order, recoverable Production state and reviewable evidence while disclosing the one unexecuted TTS request and the normal-page-load data side effect without inventing a pass.
...
- Backup runner incident: two pre-change backup attempts completed the read-only Production dump but hung before isolated restore because captured `pg_ctl start` output kept its pipe open. The exact isolated clusters were stopped; both temporary directories containing the short-lived identity file and both unverified archives were deleted. The runner now gives `pg_ctl` ignored stdio and must produce explicit restore evidence before deployment can proceed.
- Restore diagnosis: the next fail-closed attempt reached `pg_restore`, which rejected the archive because the command helper changed requested `null` encoding back to UTF-8 and corrupted the binary custom dump/decrypt Buffer. Final cleanup removed the temporary cluster/identity and unverified archive. The helper now distinguishes `undefined` from `null`, and a focused static regression pins binary Buffer preservation plus all three ignored-stdio `pg_ctl` call sites.
