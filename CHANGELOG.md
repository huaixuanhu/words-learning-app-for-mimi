...
- The first Staging inventory stopped before database connection because the retained branch had been automatically archived for inactivity. A credential-safe control-plane diagnostic confirmed the same `staging` name, direct `main` parent, endpoint and Sydney region. The guard now permits only that exact archived Staging identity to be accessed, then requires automatic unarchive to reach `ready`; Production `main` must always start `ready`, and no branch creation/restoration/rename/deletion is permitted.
- The next read-only Staging attempt successfully woke and connected to the exact branch, then stopped because the historical safe-inventory artifact allowlist had no `staging` label. The shared artifact now accepts `staging` for read-only evidence; historical remote-write commands retain their explicit clone/main target arrays and gain no Staging mutation authority.
...
- Focused V2.2 product tests passed 12 files / 80 tests; the Staging/Production release guards pass 3 files / 20 tests. With the security-refreshed lockfile and archived-Staging repair, full Vitest passed 98 files / 607 tests with the existing Postgres integration file/test skipped; ESLint, TypeScript, all three backup dry-runs, Next.js Production build, Tier 3 governance checks, full/Production npm audits and diff checks pass. No migration, backup, provider request, Git push or Vercel activation has occurred.
...
- Local release validation passes 96 test files / 589 tests with the existing Postgres integration file/test skipped; lint, TypeScript, all three backup dry-runs, Production build and Production dependency audit with 0 vulnerabilities also pass.
- The first two pre-change backup attempts exposed a local runner defect: captured `pg_ctl start` output kept the parent pipe open. Both attempts were stopped before restore, their exact temporary identity directories and unverified archives were removed, and the runner now starts/stops isolated PostgreSQL with ignored stdio so the parent can continue.
- The next fail-closed attempt reached `pg_restore` and exposed a second local-only defect: the helper converted binary `pg_dump`/decrypt output to UTF-8 because `null` encoding fell through to the text default. Its temporary resources and unverified archive were removed; the helper now preserves `null` as a Buffer and a focused regression test pins both backup-runner boundaries.
