...
Status: V2-0 through V2-8-2.3 are complete within their approved scopes. V2-8-2.1 is separately verified on exact-commit protected Preview; V2-8-2.2 is complete and its `0004` data addition is present on non-Production `staging`; PF-001 is `High / Closed`. Google Cloud Standard pins `en-AU-Standard-C` at `0.9` speaking rate, `0` pitch and MP3. The accepted local result remains 48 recorded ratings: 46 Good, 2 Review, 0 Bad, with two unrecorded entries and minor stress weakness retained as limitations. Gate E established Preview-only WIF without a long-lived key, migrated `staging` with `0004 -> 0005`, deployed exact commit `deab32f3ab96025116597881b7b69c9dde84b8f4`, and passed Kill Switch plus machine-route checks. On 2026-07-22 the user completed MacBook + Chrome human playback across Settings, Recognition, Active and example-word routes, confirmed the issue resolved and described the audio as very ideal. The record does not claim an independent Mimi pass or iPhone + Safari compatibility. The accepted names are `V2-7B-1` and `V2-7B-2`. Branch `V2` remains Schema Version 6 and JSON backup Version 4. The retained `staging` Schema 5 recovery checkpoint remains available until `2026-08-17T12:00:00Z` unless separately extended. Production AI/TTS remain closed; live V1 stays Schema Version 5. V2-8-3 Gate 1 and Gate 0B are complete. Gate 2 is the next candidate stage and still requires fresh approval. The whole V2 release excludes SSO and confidential per-person authorization.
...
- V2-8-2.3 closed after the protected Preview ran the complete candidate with required migrations, its only `High` item closed, and the missing independent Mimi / iPhone + Safari results were recorded as evidence limitations. Its completion authorizes no Production migration, deployment, credential change or provider action.
...
- Gate 0B is complete and stopped without alias/environment/credential/Production changes. With V2-8-2.3 complete, Gate 2 is the next candidate remote work but still requires fresh approval. It owns current official-fact refresh and metadata-only account inventory before later approvals for backup, migration, secrets, deployment, authenticated smoke, schema/data/log/budget acceptance and rollback readiness.
