...
Status: V2-0 through the historical PF-001 V2-8-2.3 closeout are complete within their approved scopes. V2-8-2.1 is separately verified on exact-commit protected Preview; V2-8-2.2 is complete and its `0004` data addition is present on non-Production `staging`; PF-001 is `High / Closed`. Google Cloud Standard pins `en-AU-Standard-C` at `0.9` speaking rate, `0` pitch and MP3. The user passed MacBook + Chrome playback and Mimi later confirmed the sound had no problem, while her exact device/browser is unrecorded and iPhone + Safari remains untested. PF-002 is `Normal / Preview-ready` and awaits human retest. PF-003 is `Normal / Fixed locally` and awaits commit, protected Preview and human retest. The accepted names are `V2-7B-1` and `V2-7B-2`. Branch `V2` remains Schema Version 6 and JSON backup Version 4. The retained `staging` Schema 5 recovery checkpoint remains available until `2026-08-17T12:00:00Z` unless separately extended. Production AI/TTS remain closed; live V1 stays Schema Version 5. V2-8-3 Gate 1 and Gate 0B are complete. Gate 2 is paused until all open PF closure/acceptance and later fresh approval. The whole V2 release excludes SSO and confidential per-person authorization.
...
- `PF-002` is `Normal / Preview-ready`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`: learner-facing navigation now uses `Review / New Learning`, Home and Study foreground only the two configurable goals, session values use explicit daily/ready labels, and the desktop summary panels align. Instrument Serif has been removed after the user found it uncomfortable; titles use Geist semibold and large vocabulary restores the pre-PF-002 Georgia fallback. Existing data definitions, Motion, FSRS, API, Schema, AI and TTS contracts are unchanged.
...
- `PF-003` is `Normal / Fixed locally` and remains in the parent register because it is a bounded parity fix. Active New Learning/Review and Say it/Spell it/Dictation now share the Recognition completion-dialog behavior: after the final accepted queue result, Done closes the dialog and conditionally plays the existing local Mimi completion sound under the existing UI-only preference. Focused Active coverage passes 1 file / 4 tests; full Vitest passes 91 files / 554 tests with the existing Postgres integration file/test skipped; lint, typecheck, all three backup dry-runs and Production build pass. No learning, scheduling, persistence, API, TTS, Motion or sound-asset contract changes.
...
- Gate 0B is complete and stopped without alias/environment/credential/Production changes. Gate 2 is paused while PF-002 or PF-003 remains open; after all open items close or are explicitly accepted, Gate 2 would still require fresh approval. It owns current official-fact refresh and metadata-only account inventory before later approvals for backup, migration, secrets, deployment, authenticated smoke, schema/data/log/budget acceptance and rollback readiness.
