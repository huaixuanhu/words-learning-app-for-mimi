...
Status: V2-0 through the historical PF-001 V2-8-2.3 closeout are complete within their approved scopes. V2-8-2.1 is separately verified on exact-commit protected Preview; V2-8-2.2 is complete and its `0004` data addition is present on non-Production `staging`; PF-001 is `High / Closed`. Google Cloud Standard pins `en-AU-Standard-C` at `0.9` speaking rate, `0` pitch and MP3. The user passed MacBook + Chrome playback and Mimi later confirmed the sound had no problem, while her exact device/browser is unrecorded and iPhone + Safari remains untested. PF-002 is `Normal / Preview-ready`: its historical exact Preview passed responsive/runtime machine checks, but the user rejected the Instrument Serif experience; the Geist rollback is now Ready on a new exact protected Preview and passes bounded desktop machine checks. Human retest remains pending. The accepted names are `V2-7B-1` and `V2-7B-2`. Branch `V2` remains Schema Version 6 and JSON backup Version 4. The retained `staging` Schema 5 recovery checkpoint remains available until `2026-08-17T12:00:00Z` unless separately extended. Production AI/TTS remain closed; live V1 stays Schema Version 5. V2-8-3 Gate 1 and Gate 0B are complete. Gate 2 is paused until PF-002 closure/acceptance and later fresh approval. The whole V2 release excludes SSO and confidential per-person authorization.
...
- `V2-8-2.3` completed its historical PF-001 closeout on 2026-07-22, then reopened for PF-002. Its canonical child plan is `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; the old Instrument Preview machine retest is retained as history, the Geist rollback exact Preview is Ready, and the stage remains active until human closure or explicit acceptance.
