...
Status: V2-0 through V2-8-2 are complete within their approved scopes; V2-8-2.1 is complete and separately verified on exact-commit protected Preview; V2-8-2.2 is complete locally and its `0004` data addition is now present on non-Production `staging`; V2-8-2.3 is active with PF-001 `High / Preview-ready`. Google Cloud Standard is selected and the candidate pins `en-AU-Standard-C` at `0.9` speaking rate, `0` pitch and MP3. The strict route, explicit browser fallback, independent TTS limits/Cache and additive `0005` exist; user-approved ADC proof and local human corpus acceptance succeeded. The accepted local result is 48 recorded ratings: 46 Good, 2 Review, 0 Bad; two unrecorded entries and the minor stress weakness remain explicit limitations. Gate E established a Preview-only WIF identity without a long-lived key and migrated `staging` with exact `0004 -> 0005`; the deployed protected Preview still runs the previous application until the exact candidate is deployed. The accepted names are `V2-7B-1` and `V2-7B-2`. Branch `V2` remains Schema Version 6 and JSON backup Version 4. The retained `staging` Schema 5 recovery checkpoint remains available until `2026-08-17T12:00:00Z` unless separately extended. The protected `V2` Preview retains its existing Gemini controls and accepted ledger of 3 successful synthetic attempts, 1,467 tokens, and `US$0.000777`; no Preview TTS call has yet been accepted as Gate E evidence. Production AI/TTS remain closed; the live V1 database stays Schema Version 5. V2-8-3 Gate 1 and Gate 0B are complete. Gate 2 is paused until V2-8-2.3 closes and still requires fresh approval; no Production account/database read, migration, credential change, deployment or write occurred. The whole V2 release excludes SSO and confidential per-person authorization.
...
- `PF-001` is the first `High` finding and is now `Preview-ready`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_1_GOOGLE_CLOUD_STANDARD_TTS.md`: Google Cloud Text-to-Speech Standard is the default candidate for all playback routes, browser speech is an explicit fallback, user audition fixed `en-AU-Standard-C` / `0.9` / pitch `0` / MP3 as `google-en-au-standard-c-v1`, and Gate E has established Preview-only WIF plus `staging` `0004` / `0005` readiness before application deployment.
