...
- `V2-8-2.3` completed its historical PF-001 closeout on 2026-07-22, then reopened for PF-002. Its canonical child plan is `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; it remains active until PF-002 receives protected Preview retest and human closure or explicit acceptance.
...
- `PF-002` is `Normal / Fixed locally`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`: learner-facing navigation now uses `Review / New Learning`, Home and Study foreground only the two configurable goals, session values use explicit daily/ready labels, the desktop summary panels align, and Instrument Serif is limited to English display titles and large vocabulary text. Existing data definitions, Motion, FSRS, API, Schema, AI and TTS contracts are unchanged.
- PF-002 local acceptance passes focused/full tests, lint, typecheck, three backup dry-runs, Production build and 1280/390 px browser checks. It remains open because no exact protected Preview deployment or human retest belongs to this local batch.
