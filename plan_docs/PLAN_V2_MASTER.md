...
- `V2-8-2.3` completed its historical PF-001 closeout on 2026-07-22, then reopened for PF-002. Its canonical child plan is `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; the old Instrument Preview machine retest is retained as history, and the stage remains active until a new exact Preview and human closure or explicit acceptance.
...
- `PF-002` is `Normal / Fixed locally`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`: learner-facing navigation now uses `Review / New Learning`, Home and Study foreground only the two configurable goals, session values use explicit daily/ready labels, and the desktop summary panels align. Instrument Serif has been removed after the user found it uncomfortable; titles use Geist semibold and large vocabulary restores the pre-PF-002 Georgia fallback. Existing data definitions, Motion, FSRS, API, Schema, AI and TTS contracts are unchanged.
- Historical PF-002 local/Preview acceptance passes focused/full tests and the 1280/390 px machine checks on deployment `dpl_DxQWHVspQuukaeR6LTCjKp15a3zp` / commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131`. That deployment is no longer the current candidate because it contains the rejected font. The font-only rollback contract passes 1 file / 3 tests plus lint, typecheck, Production build, governance and diff checks; it requires a fresh exact protected Preview plus human retest or explicit acceptance.
