...
- `PF-003` is `Normal / Preview-ready` and remains in the parent register because it is a bounded parity fix. Active New Learning/Review and Say it/Spell it/Dictation now share the Recognition completion-dialog behavior: after the final accepted queue result, Done closes the dialog and conditionally plays the existing local Mimi completion sound under the existing UI-only preference. Focused Active coverage passes 1 file / 4 tests; full Vitest passes 91 files / 554 tests with the existing Postgres integration file/test skipped; lint, typecheck, all three backup dry-runs and Production build pass. Exact commit `acd009cc3879275dffa1d22c470b6c82fd1f8263` is Ready as protected Preview deployment `dpl_BAYB3tDx1m4ybKFRtw9rdf32jZon`; human sound retest remains pending. No learning, scheduling, persistence, API, TTS, Motion or sound-asset contract changes.
