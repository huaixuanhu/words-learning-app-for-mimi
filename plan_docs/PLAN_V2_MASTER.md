...
- `V2-8-2.3` completed its historical PF-001 closeout on 2026-07-22, then reopened for PF-002. Its canonical child plan is `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; exact protected Preview machine retest is complete and the stage remains active until human closure or explicit acceptance.
...
- `PF-002` is `Normal / Preview-ready`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`: learner-facing navigation now uses `Review / New Learning`, Home and Study foreground only the two configurable goals, session values use explicit daily/ready labels, the desktop summary panels align, and Instrument Serif is limited to English display titles and large vocabulary text. Existing data definitions, Motion, FSRS, API, Schema, AI and TTS contracts are unchanged.
- PF-002 local acceptance passes focused/full tests, lint, typecheck, three backup dry-runs, Production build and 1280/390 px browser checks. Exact protected Preview deployment `dpl_DxQWHVspQuukaeR6LTCjKp15a3zp` binds commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131`, reports Ready/SYD1, retains Deployment Protection, and passes the same responsive/hierarchy/font checks with zero browser warning/error and zero Vercel Warning/Error/Fatal entries in the acceptance window. It remains open only for human retest or explicit acceptance.
