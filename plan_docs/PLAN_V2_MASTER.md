...
Status: V2-0 through V2-8-2 are complete within their approved scopes; V2-8-2.1 is complete and separately verified on exact-commit protected Preview; V2-8-2.2 is complete and its `0004` data addition is present on non-Production `staging`; V2-8-2.3 is active with PF-001 `High / Preview-ready`. Google Cloud Standard is selected and pins `en-AU-Standard-C` at `0.9` speaking rate, `0` pitch and MP3. The accepted local result is 48 recorded ratings: 46 Good, 2 Review, 0 Bad; two unrecorded entries and the minor stress weakness remain explicit limitations. Gate E established Preview-only WIF without a long-lived key, migrated `staging` with `0004 -> 0005`, deployed exact commit `deab32f3ab96025116597881b7b69c9dde84b8f4`, and passed Kill Switch plus Settings/Recognition/example-word/Active Dictation machine checks. Its ledger records 7 attempts, 170 characters and `US$0.000680` list-price equivalent, including three safely retained diagnostic failures and four successful route calls; Cache replay did not add an attempt. The accepted names are `V2-7B-1` and `V2-7B-2`. Branch `V2` remains Schema Version 6 and JSON backup Version 4. The retained `staging` Schema 5 recovery checkpoint remains available until `2026-08-17T12:00:00Z` unless separately extended. Production AI/TTS remain closed; live V1 stays Schema Version 5. V2-8-3 Gate 1 and Gate 0B are complete. Gate 2 is paused until PF-001 receives user/Mimi real-device closure and V2-8-2.3 closes; it still requires fresh approval. The whole V2 release excludes SSO and confidential per-person authorization.
...
- `PF-001` is the first `High` finding and remains `Preview-ready`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_1_GOOGLE_CLOUD_STANDARD_TTS.md`: Google Cloud Text-to-Speech Standard is the default for all playback routes, browser speech is an explicit fallback, and user audition fixed `en-AU-Standard-C` / `0.9` / pitch `0` / MP3 as `google-en-au-standard-c-v1`. Gate E has completed Preview-only WIF, `staging` `0004` / `0005`, exact application deployment, Kill Switch and machine-route checks; user/Mimi human retest remains open.
