...
Status: V2-0 through V2-8-2 are complete within their approved scopes; V2-8-2.1 is complete and separately verified on exact-commit protected Preview; V2-8-2.2 is complete in the local branch only; V2-8-2.3 is active with a documentation-defined intake and closure contract. The accepted names are `V2-7B-1` and `V2-7B-2`. Branch `V2` now uses Schema Version 6 plus additive migration `0004` and JSON backup Version 4. Long-lived non-Production `staging` and the deployed protected Preview still use the pre-V2-8-2.2 Schema 6 / backup Version 3 contract until a separately approved migration and deployment. The retained `staging` Schema 5 recovery checkpoint was rechecked on 2026-07-19 and remains available until `2026-08-17T12:00:00Z` unless separately extended. The protected `V2` Preview has its own study-token secret, Gemini Auth Key, `postgres-preview` target, full UI writes, Vercel Authentication, global cost controls, and Kill Switch. Its accepted Gemini ledger remains 3 successful synthetic attempts, 1,467 tokens, and `US$0.000777`; Gate 0B made zero provider call and did not re-query that ledger. Production AI remains closed; the live V1 database stays Schema Version 5. V2-8-3 Gate 1 and Gate 0B are complete. Gate 2 is paused until V2-8-2.3 closes and will still require fresh approval; no Production account/database read, migration, credential change, deployment or write has occurred. The whole V2 release excludes SSO and confidential per-person authorization.
...
- `V2-8-3` has a canonical child plan at `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md`. Local Gate 1 and separately approved Gate 0B are complete. Gate 1 provides exact cutover modes, old-client write rejection, dormant non-empty Schema 5 / Schema 6 clone/main tools, authenticated live Neon control-plane endpoint/branch binding before any Postgres connection, a Gate-2-owned Production-project hash pin that intentionally remains unset, separately pinned `0003` and additive `0004` SHA-256 values, and a secret-free cutover manifest with paired rollback evidence. Gate 0B independently proves the earlier performance artifact on protected Preview without entering Production.
