...
Document nature:
This is the canonical V2 product and engineering master plan. It is derived from the completed V1 plan and current Production architecture. V2-1 through V2-8-2.3 completed the product, scheduling, Active, AI, mobile, Dashboard, performance, audio, bilingual-example and Preview feedback layers. PF-001 is `High / Closed`; PF-002 and PF-003 are `Normal / Closed by explicit acceptance`. V2-8-3 Gates 0B–6 then completed the guarded Production release: two non-empty clone migrations, independent encrypted recovery, old V1 credential revocation, atomic `0003 -> 0004 -> 0005` migration, full core-data parity, exact `main` deployment, real Gemini/TTS smoke and steady-state accounting. V2 now serves the canonical Production domain. Gate 7 post-launch stability observation remains open until a real learning run by both users can be recorded. V2.2 activated in Production after the pinned constraint-only `0007` Staging rehearsal, Production backup/restore, migration and exact Git/Vercel activation. V2.3 is now active under the user-authorized release owned by `plan_docs/PLAN_V2_3_STORAGE_STUDY_DAY_HOME.md` Stage 2.3.12. The retained V2.2 evidence gap is a fresh exact-text `review` TTS request; no provider or credential setting was changed to manufacture that evidence.
...
Status: V2.3 is deployed in Production; database access recovered after the user completed Neon Launch on 2026-09-13. The V2.3 child plan owns safeguards and backup activation status; Stage 2.3.12 owns the authorized release, exact Git/Vercel identities and completed or pending online checks. V2-0 through V2-8-2.3, V2-8-3 Gates 0B–6 and the V2.2 migration-first release are complete at application activation. `main` remains Schema Version 6 with JSON backup Version 4; `0007` now accepts only the matching V1/V2 Recognition and Active Parameter Set identifiers. Google Cloud Standard remains pinned to `en-AU-Standard-C` at `0.9` speaking rate, `0` pitch and MP3; paid `gemini-3.1-flash-lite` remains behind the accepted disclosure and global cost guards. The canonical deployment is in `syd1`, Basic Auth remains active, and migration-time study data retained exact parity. The retained recovery points and encrypted pre/post V2.2 backups remain available. The whole V2 release excludes SSO and confidential per-person authorization. The fresh exact-text V2.2 TTS acceptance request is explicitly unexecuted; historical Production TTS proof and current local source-routing coverage are retained.

## V2.3 Current Release

2026-09-13，用户要求建立 V2.3 分支，调查 Library 空白、将学习日改到明确选定的凌晨 06:00，并简化 Home Insight；Memory outlook 后续明确选择保留分组小图。唯一当前修复计划为 `plan_docs/PLAN_V2_3_STORAGE_STUDY_DAY_HOME.md`。用户随后明确授权本地提交及读取现有连接凭据进行只读核查；初始修复已提交为 `0f242d5`。Neon 直接返回网络传输额度超限，当时 Free 项目本月已传出约 6.33 GB；用户完成 Launch 升级后，固定 Production 身份的只读核查恢复，确认 5,585 个词、1,194 条复习事件，所检重复/孤立关系为 0。连接加固与传输放大风险分别记录。用户随后授权按需读取、防重复保存、防查询循环、常态化加密备份及现有 Neon Launch 基础套餐升级；Launch 已生效，最新备份选择为 Neon 云端每日快照、本机每月独立加密副本，并设置 US$5 提醒。后续实施与激活状态由上述唯一 V2.3 计划 Stage 2.3.11 统一拥有。用户已授权完整推送与部署，Production 已运行 V2.3；精确发布身份及已完成和待完成的线上验收由 V2.3 计划 Stage 2.3.12 拥有。数据库迁移、学习记录编辑和恢复不在此次发布范围。
...
- This parent plan did not by itself authorize remote work. The later user authorization and V2-8-3 child plans record the exact Production actions that were completed.
- No unlisted Production mutation, public registration, broader provider authority or post-launch cleanup follows from the completed release.
...
- V2-8-3 Production uses the exact cutover modes `maintenance`, `schema6-readiness`, and `live`; a missing or unknown mode fails closed. Basic Auth is evaluated before maintenance, and only `live` accepts Production mutations carrying the code-owned `v2-schema6` client contract. Generic `/api/storage/data` POST additionally requires `x-mimi-storage-client-revision: v2.3`; missing or incompatible revisions receive `428` before repository access.
...
V2-3 established Schema Version 6 and JSON backup Version 3. V2-8-2.2 keeps the Schema Version 6 name, adds the immutable forward-only `0004_v2_bilingual_examples.sql`, and advances new JSON backups to Version 4 while continuing to read Versions 1–3. `0003`, `0004` and PF-001 `0005` are applied to long-lived non-Production `staging` and Production `main`.
...
- V2-8-2.2 raises JSON backup to Version 4 and adds the forward-only Schema 6 migration `0004_v2_bilingual_examples.sql`, pinned to SHA-256 `9e00e1213366492db90a97709d605d68770a1b80f46caa8b748886bc8bd4e29c`. Existing English-only records migrate with visible empty translation slots and must be reviewed/backfilled before being called bilingual-complete. `0004` is applied to non-Production `staging` and Production `main`.
- `V2-8-2.3` completed its historical PF-001 closeout on 2026-07-22, then reopened for PF-002 and later registered PF-003. Its canonical register is `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; the old Instrument Preview machine retest is retained as history, the Geist rollback exact Preview is Ready, and the stage remains active until all open items receive human closure or explicit acceptance.
- `PF-001` is `High / Closed`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_1_GOOGLE_CLOUD_STANDARD_TTS.md`: Google Cloud Text-to-Speech Standard is the default for all playback routes, browser speech is an explicit fallback, and user audition fixed `en-AU-Standard-C` / `0.9` / pitch `0` / MP3 as `google-en-au-standard-c-v1`. Preview-only WIF, `staging` `0004` / `0005`, exact application deployment, Kill Switch, machine-route checks and MacBook + Chrome human playback all passed. Mimi later independently confirmed that the sound had no problem, although her exact device/browser was not recorded; iPhone + Safari remains untested.
- PF-001 implements a separate global TTS boundary of 2,000 provider attempts/day, 100,000 characters/day, 1,000,000 characters/month, conservative list-price-equivalent `US$0.50/day` / `US$4/month`, concurrency 4, Cache and Kill Switch, with no per-person limit. Local proof uses user ADC; Preview and Production use separate exact project/environment-bound WIF identities. Production Standard-C synthesis and Cache passed.
- `PF-002` is `Normal / Closed by explicit acceptance`. Its derived plan is `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`: learner-facing navigation uses `Review / New Learning`, Home and Study foreground only the two configurable goals, session values use explicit daily/ready labels, and the desktop summary panels align. Instrument Serif was removed after the user found it uncomfortable; titles use Geist semibold and large vocabulary restores the pre-PF-002 Georgia fallback. The user accepted the exact Geist candidate without a new comfort-retest claim. Existing data definitions, Motion, FSRS, API, Schema, AI and TTS contracts are unchanged.
- Historical PF-002 local/Preview acceptance passes focused/full tests and the 1280/390 px machine checks on deployment `dpl_DxQWHVspQuukaeR6LTCjKp15a3zp` / commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131`; that deployment is superseded because it contains the rejected font. The font-only rollback contract passes 1 file / 3 tests plus lint, typecheck, Production build, governance and diff checks. Its current protected Preview is deployment `dpl_EmRVmp5YDTKgnh1VEtVBozwhRp6J` / exact commit `54c8ca4492ff9b13d095ea0bd0d3d7ca702c3c2f`; desktop machine checks pass, no new remote 390 px check is claimed, and human retest or explicit acceptance remains required.
- `PF-003` is `Normal / Closed by explicit acceptance` and remains in the parent register because it is a bounded parity fix. Active New Learning/Review and Say it/Spell it/Dictation share the Recognition completion-dialog behavior: after the final accepted queue result, Done closes the dialog and conditionally plays the existing local Mimi completion sound under the existing UI-only preference. Focused Active coverage passes 1 file / 4 tests; full Vitest passes 91 files / 554 tests with the existing Postgres integration file/test skipped; lint, typecheck, all three backup dry-runs and Production build pass. Exact commit `acd009cc3879275dffa1d22c470b6c82fd1f8263` is Ready as protected Preview deployment `dpl_BAYB3tDx1m4ybKFRtw9rdf32jZon`; the user accepted it without a new final-card listening claim. No learning, scheduling, persistence, API, TTS, Motion or sound-asset contract changes.
...
- V2.2 completed local product acceptance on 2026-09-03 and Production activation on 2026-09-04 under `plan_docs/PLAN_V2_2_REVIEW_CADENCE_CARD_AUDIO.md` and `plan_docs/PLAN_V2_2_PRODUCTION_RELEASE.md`. New ratings use versioned `recognition-fsrs-v2` / `active-fsrs-v2` parameters, V1 history remains replayable, the first new-word `vague` checkpoint remains the next local day, the full muted Recognition surface toggles the card, and newly active Recognition/Dictation cards request one playback while manual buttons remain.
- Pinned `0007` completed once on Staging and once on Production with matching constraints, unchanged migration-time 22-table digests and zero checked invariants. Candidate `727a7089acbaa51fb00820dea7d39fc7aaa543d1` became the Ready/Current `syd1` Production application; pre/post encrypted backups passed isolated PostgreSQL 17 restore. Basic Auth, authenticated core pages, storage health and error scans passed. A fresh exact-text TTS request was not sent and is not claimed as passed.
...
This master plan records the completed V2.1 and V2.2 Production releases and current V2.3 activation. The user-authorized V2.3 release and its exact deployment/acceptance state are owned by `plan_docs/PLAN_V2_3_STORAGE_STUDY_DAY_HOME.md` Stage 2.3.12. The first AI-enabled product version, independent Active practice, daily planning, mobile refinement, bilingual examples, Standard-C audio and evidence-based Dashboard run in Production. V2.2 review cadence, whole-surface card interaction and autoplay/source repair are active; their exact migration, backup, Git, Vercel and bounded-verification states are owned by `plan_docs/PLAN_V2_2_PRODUCTION_RELEASE.md`. V1 remains paired recovery evidence; Gate 7 owns the honest natural-day/two-user stability observation and later cleanup.
