...
Document nature:
This is the canonical V2 product and engineering master plan. It is derived from the completed V1 plan and current Production architecture. V2-1 has an isolated executable contract; V2-2 / 2-B have bounded local AI quality evidence; V2-3 has implemented the local/application Schema Version 6 and backup parity; V2-3.1 and V2-4 have implemented the bounded local Review-interaction and mobile/copy layers; V2-5 / 5.1 have connected daily plans and repaired cross-day scheduling; V2-6 has implemented the independent Active practice engine. V2-7A completed the browser-local fixture product flow and dormant safety layer, and V2-7B-1 / V2-7B-2 completed formal orchestration and a disposable provider proof. V2-8-1 completed Dashboard Insights, V2-8-1.1 completed learner copy, V2-8-2 completed protected Preview rehearsal, V2-8-2.1 stabilised performance and V2-8-2.2 completed keyboard/bilingual-example work. PF-001 completed the first V2-8-2.3 closeout as `High / Closed`; Mimi later confirmed its audio quality without an exact environment record, and iPhone + Safari remains untested. On 2026-07-22 the user explicitly accepted PF-002 and PF-003 as `Normal / Closed by explicit acceptance`; their exact protected Preview and machine evidence remain valid without claiming new human comfort/listening runs. V2-8-2.3 is complete again. V2-8-3 Gate 0B, local Gate 1, remote read-only Gate 2 and encrypted backup/restore Gate 3 are complete. Exact commit `88ddb1c2c96437ca3cc4a8f2103990ae70e79eac` has a repository-external encrypted Production Schema 5 archive with successful isolated PostgreSQL 17 restore parity. Gate 4 has explicit user approval and a derived execution plan; it is paused at its independent API-key credential checkpoint before any clone creation. Production V1, Neon `main` and Schema 5 data remain unchanged.
...
Status: V2-0 through V2-8-2.3 are complete within their approved scopes. V2-8-2.1 is separately verified on exact-commit protected Preview; V2-8-2.2 is complete and its `0004` data addition is present on non-Production `staging`; PF-001 is `High / Closed`. Google Cloud Standard pins `en-AU-Standard-C` at `0.9` speaking rate, `0` pitch and MP3. The user passed MacBook + Chrome playback and Mimi later confirmed the sound had no problem, while her exact device/browser is unrecorded and iPhone + Safari remains untested. PF-002 and PF-003 are `Normal / Closed by explicit acceptance`; no new human comfort/listening run is claimed. The accepted names are `V2-7B-1` and `V2-7B-2`. Branch `V2` remains Schema Version 6 and JSON backup Version 4. The retained `staging` Schema 5 recovery checkpoint remains available until `2026-08-17T12:00:00Z` unless separately extended. Production AI/TTS remain closed; live V1 stays Schema Version 5. V2-8-3 Gate 1, Gate 0B, remote read-only Gate 2 and encrypted backup/restore Gate 3 are complete. Exact commit `88ddb1c2c96437ca3cc4a8f2103990ae70e79eac` has a restore-verified encrypted Production Schema 5 archive outside the repository; Production remained read-only. Gate 4 is approved and paused at Gate 4A because no approved `NEON_API_KEY` exists in the process environment or dedicated Keychain entry. No clone has been created. The whole V2 release excludes SSO and confidential per-person authorization.
...
- V2-8-2.2 raises JSON backup to Version 4 and adds the forward-only Schema 6 migration `0004_v2_bilingual_examples.sql`, pinned to SHA-256 `9e00e1213366492db90a97709d605d68770a1b80f46caa8b748886bc8bd4e29c`. Existing English-only records migrate with visible empty translation slots and must be reviewed/backfilled before being called bilingual-complete. Gate E subsequently applied `0004` to non-Production `staging` and deployed its application path on protected Preview; Production remains unchanged.
...
- `V2-8-3` has a canonical child plan at `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md`. Local Gate 1, separately approved Gate 0B and separately approved remote read-only Gate 2 are complete. Gate 3 is separately approved and derives to `plan_docs/PLAN_V2_STAGE8_3_GATE3_ENCRYPTED_LOGICAL_BACKUP_RESTORE_REHEARSAL.md`; PostgreSQL 17/`age`, Keychain custody, stream encryption, negative tests and synthetic restore parity pass locally. The first clean-commit Production runner stopped before archive creation because its full URL was incorrectly assigned to `PGDATABASE`; the corrected standard libpq field mapping passes a minimal Production metadata query and focused regression coverage. The real backup remains blocked until that repair is a clean exact commit. Gate 2 pins only the independently verified Production project SHA-256; all clone/main commands remain dormant behind authenticated Neon control-plane identity, backup/clone/write-free gates and human confirmation. The migration sequence is fixed as `0003 -> 0004 -> 0005`.
...
- Gate 0B and Gate 2 completed without mutation. The user explicitly accepted PF-002 / PF-003 and separately approved Gate 3. Gate 3 may consume one Production `main` unpooled connection string copied read-only from the signed-in Neon Console, clear it from the clipboard immediately, create one encrypted repository-external backup and restore it only into an isolated local PostgreSQL 17 cluster. Vercel Sensitive values are not treated as readable. Gate 4 still blocks Neon clone creation, migration, environment/credential mutation, deployment, authenticated Production smoke, provider opening and rollback actions until a new approval.
