...
- Stage 6B execution planning is documented in `plan_docs/PLAN_V1_STAGE6B_PRODUCTION_EXECUTION.md`; on 2026-07-08 the user chose shared Postgres Production for formal V1, so `plan_docs/PLAN_V1_STAGE6B_P1_POSTGRES_PRODUCTION_RUNTIME.md` is the required runtime bridge before merge, Production migration, and deployment. Stage 8 is accepted, Stage 6B-P1-B has added the local schema version 5 migration draft, Stage 6B-P1-C has added the local `postgres-production` runtime / API contract, Stage 6B-P1-D has added local repository parity, Stage 6B-P1-E has added local backup import version 5 support, Stage 6B-P1-F has validated the migrated non-production development database, and Stage 6B-P1-G-A has documented the Production handoff. The 2026-07-10 initial-data decision starts Production empty, skips development data copy/formal backup import, and begins formal data only after cloud-backed V1 launch. P1-G-B read-only Production inventory is the next separately approved step.
