...
- Support manual entry and batch import. The current Stage 7.9 user-facing import shape is Single input plus Batch JSON import; older `.txt` parser behavior remains compatibility code.
...
- Browser-local storage now uses schema version 4 after Stage 7.9.
...
- Schema version 4 adds vocabulary `learningTrack`, nullable `tags`, `recognitionSessionLimit`, and `activeSessionLimit`.
- JSON backup exports schema version 4 while still restoring schema version 2 / 3 backups through migration.
