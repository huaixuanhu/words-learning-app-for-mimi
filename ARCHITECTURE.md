...
Formal private V1 is live on Neon `main` Schema Version 5. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`; the Stage 6B-P1-G-C-5 record is the current release source of truth. V2-8-2.3 is active. PF-001 is `High / Preview-ready`: its local route, fixture, Google adapter, Cache, accounting and `0005` are implemented. The user selected `en-AU-Standard-C`; local human evidence accepts the voice with 46 Good, 2 Review, 0 Bad and 2 unrecorded samples, retaining minor stress weakness as an explicit limitation. Protected Preview / `staging` still run the prior deployment and data contract; Production remains V1 on Schema 5. V2-8-3 Gate 2 stays paused and requires fresh approval after V2-8-2.3 closes. The whole V2 release excludes SSO（Single Sign-On，单点登录）and confidential per-person authorization.
...
The cloud-backed V1 described above remains the current Production behavior. On branch `V2`, Stage 1 through V2-8-2 built and rehearsed the complete V2; V2-8-2.1 stabilised runtime performance and V2-8-3 Gate 0B verified its exact commit on protected Preview in `SYD1`. V2-8-2.2 adds reliable keyboard rating selection, device-selectable fallback voices, paired English/Chinese examples, JSON backup Version 4 and additive Schema 6 migration `0004`; this slice is local only. V2-8-2.3 owns the numbered pre-launch feedback loop. PF-001 is `High / Preview-ready` with exact Standard-C local playback, additive `0005` and accepted local human listening evidence; its protected Preview proof remains pending. V2-8-3 local Gate 1 and remote Gate 0B remain complete, while Gate 2 waits for V2-8-2.3 closure and a later fresh approval. Production V1, Neon `main`, Production secrets, and Production Schema 5 data remain unchanged; no V1 patch is planned.
...
V2-8-2.3 is the release-quality process contract for protected Preview findings: each accepted issue receives a `PF` identifier, reproduction evidence, priority, bounded local fix, exact-commit Preview retest and human closure result. PF-001 is its first runtime/data addition and remains local-only. The current Preview must receive required migrations before its application refresh. Every `Blocker` and `High` item must close before Gate 2 can be reconsidered.

The PF-001 local implementation keeps one client speech facade across Recognition pronunciation, Active revealed answers, Dictation, selected example words and Settings preview. Strict `POST /api/tts` accepts only versioned same-origin JSON, a server-owned purpose, request UUID and normalized English text up to 240 Unicode characters; clients cannot choose a voice, provider, encoding or SSML. Runtime Cache is checked before an atomic reservation, equal misses are coalesced, and TTS attempts/characters/list-price cost/concurrency remain independent from the Gemini ledger. Current code enforces 2,000 provider attempts/day, 100,000 characters/day, 1,000,000 characters/month, `US$0.50/day`, `US$4/month`, concurrency 4 and no per-person limit. `0005_v2_standard_tts_accounting.sql` stores only environment scope, digests, counts, state and timestamps. Local fixture mode is visibly `local-fixture`; local Google mode is exact-loopback, user-ADC-only and pinned to project `for-tts-502913`. The immutable `google-en-au-standard-c-v1` contract owns `en-AU-Standard-C`, `en-AU`, speaking rate `0.9`, pitch `0` and MP3. Preferred deployed authentication remains Vercel short-lived OIDC through Google Workload Identity Federation; no service-account key exists.
