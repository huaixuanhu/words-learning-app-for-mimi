...
V2-8-2.3 owns the release-quality process contract for protected Preview findings. PF-001 is `High / Closed`: required `staging` migrations, exact application refresh, Kill Switch, machine routes and MacBook + Chrome human retest passed; Mimi later confirmed the audio without an exact environment record. PF-002 is `Normal / Fixed locally`: desktop navigation uses `Review / New Learning`, mobile uses `Learn`, both Tracks expose accessible zone tabs, Home/Study use a two-goal hierarchy, and session summaries distinguish Daily goal, today’s distinct actual and Ready now. `next/font` self-hosts Instrument Serif with Latin normal weight 400/display-swap only for semantic display headings and large vocabulary; Geist remains the body/control/data font and ChillRoundF remains the Chinese fallback. Local 1280/390 px checks have no overflow or overlay, but protected Preview deployment and human retest are pending. Gate 2 remains paused.
