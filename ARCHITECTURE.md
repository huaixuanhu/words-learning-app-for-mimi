...
The cloud-backed V1 described above remains the current Production behavior. On branch `V2`, Stage 1 through V2-8-2 built and rehearsed the complete V2; V2-8-2.1 stabilised runtime performance and V2-8-3 Gate 0B verified its exact commit on protected Preview in `SYD1`. V2-8-2.2 adds reliable keyboard rating selection, device-selectable fallback voices, paired English/Chinese examples, JSON backup Version 4 and additive Schema 6 migration `0004`; its data migration is present on non-Production `staging`. PF-001 is `High / Closed` with exact Standard-C playback, additive `0005`, local corpus evidence, keyless Preview-only WIF, exact deployment and MacBook + Chrome human acceptance. PF-002 and PF-003 are `Normal / Closed by explicit acceptance`; PF-003 repairs the Active completion-feedback omission using the existing UI-only sound contract. V2-8-3 local Gate 1, remote Gate 0B and remote read-only Gate 2 are complete. Gate 3 is approved; local backup guards and cross-timezone synthetic restore pass. A real read-only Production attempt reached isolated restore, then safely rejected a false digest difference caused by `timestamptz` display timezone; the UTC-canonical fix now waits for a clean exact commit. Production V1, Neon `main`, Production secrets, and Production Schema 5 data remain unchanged; no V1 patch is planned.
...
`plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md` is the canonical Production backup, migration and cutover plan. Local Gate 1, protected Preview Gate 0B and remote read-only Gate 2 are complete; Gate 3 is approved and its derived plan is `plan_docs/PLAN_V2_STAGE8_3_GATE3_ENCRYPTED_LOGICAL_BACKUP_RESTORE_REHEARSAL.md`:
...
- Gate 3A–3B pass tool, encryption, wrong-identity, corruption, Schema 5 restore, digest parity and cleanup checks. The first clean-commit Gate 3C runner exposed and repaired an incorrect URI-to-`PGDATABASE` mapping. The second clean-commit runner passed Production inventory, encrypted streaming and local restore, then rejected every non-empty table digest because Neon and the local cluster rendered identical `timestamptz` values in different display timezones; counts were not listed as mismatches and empty tables matched. The archive, temporary cluster and clipboard were cleaned. UTC canonicalization now passes an intentionally Melbourne-source/UTC-restore synthetic proof. Gate 3C–3D require this correction to be committed and re-proved against the new exact commit.
...
Formal private V1 is live from `main` behind application Basic Auth. Vercel Production uses `postgres-production` against Neon `main`; Development / Preview use Neon `staging` and must not promote non-production business rows. `plan_docs/PLAN_V1_STAGE6B_P1_G_C_5_LIVE_PRODUCTION_EXECUTION.md` is the current live release source of truth, and `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md` is the future V2 cutover source. Branch `V2` remains a development branch. Gate 0B, local Gate 1 and remote read-only Gate 2 are complete; V2-8-2.3 is complete after explicit PF-002 / PF-003 acceptance. Gate 3 has separate Tier 3 approval limited to read-only Production backup and isolated local restore. Local cutover tooling and runtime gates still do not authorize V2 Production migration, pull request, merge, credential mutation, Production deployment or Gate 4 work.
