...
Formal private V1 is live on Neon `main` Schema Version 5. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`; the Stage 6B-P1-G-C-5 record is the current release source of truth. V2-8-2.3 completed on 2026-07-22. PF-001 is `High / Closed`: non-Production `staging` has `0004` and `0005`, protected Preview runs the exact Standard-C candidate through keyless Preview-only WIF, machine playback passed, and the user passed Settings, Recognition, Active and example-word playback on MacBook + Chrome with “非常理想” audio. Independent Mimi and iPhone + Safari results remain unclaimed limitations. Production remains V1 on Schema 5. V2-8-3 Gate 2 is the next candidate stage and still requires fresh approval. The whole V2 release excludes SSO（Single Sign-On，单点登录）and confidential per-person authorization.
...
V2-8-2.3 completed its release-quality process contract for protected Preview findings. PF-001 was its only registered issue and is `High / Closed`: required `staging` migrations, exact application refresh, Kill Switch, machine routes and MacBook + Chrome human retest passed. The record does not claim independent Mimi or iPhone + Safari acceptance. With no open `Blocker` or `High`, Gate 2 may be reconsidered only through fresh approval.
