...
V2-8-2.3 owns the release-quality process contract for protected Preview findings. PF-001 is `High / Closed`: required `staging` migrations, exact application refresh, Kill Switch, machine routes and MacBook + Chrome human retest passed; Mimi later confirmed the audio without an exact environment record. PF-002 is `Normal / Preview-ready`: desktop navigation uses `Review / New Learning`, mobile uses `Learn`, both Tracks expose accessible zone links, Home/Study use a two-goal hierarchy, and session summaries distinguish Daily goal, today’s distinct actual and Ready now. Historical exact commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131` runs as protected Preview deployment `dpl_DxQWHVspQuukaeR6LTCjKp15a3zp` in `SYD1` and passed machine checks, but the user rejected its Instrument Serif typography after real use. The replacement exact commit `54c8ca4492ff9b13d095ea0bd0d3d7ca702c3c2f` runs as deployment `dpl_EmRVmp5YDTKgnh1VEtVBozwhRp6J`: it no longer loads Instrument Serif, display titles compute to Geist semibold, large vocabulary restores the earlier Georgia fallback, and desktop protection/region/overflow/console checks pass. Component sizing, body/control/data fonts and ChillRoundF remain unchanged. Human retest remains pending; no fresh remote 390 px claim is made, and Gate 2 remains paused.
