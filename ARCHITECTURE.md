...
Formal private V1 is live on Neon `main` Schema Version 5. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`; the Stage 6B-P1-G-C-5 record is the current release source of truth. V2-8-2.3 is active. PF-001 is `High / Preview-ready`: its route, fixture, Google adapter, Cache, accounting and `0005` are implemented. The user selected `en-AU-Standard-C`; local human evidence accepts the voice with 46 Good, 2 Review, 0 Bad and 2 unrecorded samples, retaining minor stress weakness as an explicit limitation. Non-Production `staging` now has `0004` and `0005`, and a Preview-only WIF identity exists without a long-lived key; protected Preview still runs the prior application commit pending exact deployment. Production remains V1 on Schema 5. V2-8-3 Gate 2 stays paused and requires fresh approval after V2-8-2.3 closes. The whole V2 release excludes SSO（Single Sign-On，单点登录）and confidential per-person authorization.
...
The cloud-backed V1 described above remains the current Production behavior. On branch `V2`, Stage 1 through V2-8-2 built and rehearsed the complete V2; V2-8-2.1 stabilised runtime performance and V2-8-3 Gate 0B verified its exact commit on protected Preview in `SYD1`. V2-8-2.2 adds reliable keyboard rating selection, device-selectable fallback voices, paired English/Chinese examples, JSON backup Version 4 and additive Schema 6 migration `0004`; its data migration is now present on non-Production `staging`. V2-8-2.3 owns the numbered pre-launch feedback loop. PF-001 is `High / Preview-ready` with exact Standard-C playback, additive `0005`, accepted local human listening evidence and Preview-only WIF; its exact protected Preview deployment and human proof remain pending. V2-8-3 local Gate 1 and remote Gate 0B remain complete, while Gate 2 waits for V2-8-2.3 closure and a later fresh approval. Production V1, Neon `main`, Production secrets, and Production Schema 5 data remain unchanged; no V1 patch is planned.
