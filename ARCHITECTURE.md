...
- Formal `/api/ai/enrichment` and `/api/ai/context-explain` handlers re-check Basic Auth, same-origin, JSON body size, exact public request shape, runtime mode, and Disclosure Version. Stage 7A left them resting. Completed V2-7B-1 adds server-owned item lookup, session-bound current-disclosure evidence, canonical SHA-256 hashes, Cache and complete Idempotency, same-Cache in-flight protection, atomic quota reservation, strict fake-provider orchestration, source revalidation, transactional result persistence, and server mutations for accept/reject/`Add to learning`. Its code-owned provider activation gate remains closed regardless of environment values. The Gemini adapter stays lazy, server-only, pinned, tool-free, retry-free, sends top-level `store: false`, and is guarded by a 90-second timeout. V2-7B-2 requires separate approval before a temporary non-Production Schema 6 target, Auth Key, or provider call can exist.
...
- V2-7B-2 synthetic confirmation of limited-retention disclosure and minimal outbound lexical data
- Production provider/accounting integration after V2-7B-2 and later release work are separately approved
