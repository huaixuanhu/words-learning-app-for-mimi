...
- Formal `/api/ai/enrichment` and `/api/ai/context-explain` handlers re-check Basic Auth, same-origin, JSON body size, exact public request shape, runtime mode, and Disclosure Version before returning a resting state. Server-owned item lookup and persisted current-disclosure confirmation remain Stage 7B. The Gemini adapter is lazy, server-only, pinned, tool-free, retry-free, and guarded by a 90-second timeout. Atomic Postgres global reservation/reconciliation and stale in-flight recovery are dormant-tested; missing configuration, accounting, disclosure, credential, or provider enablement fails closed. Next.js detected the ignored `.env.local`, but the agent inspected no value or credential; Stage 7A made no provider or remote-database call. Stage 7B requires current disclosure confirmation before its separately approved first real call.
...
- Production provider/accounting integration after Stage 7B is separately approved
