...
- Gate 3A–3B pass tool, encryption, wrong-identity, corruption, Schema 5 restore, digest parity and cleanup checks. The first clean-commit Gate 3C runner passed credential identity guards but stopped before backup creation because the URI was incorrectly placed in `PGDATABASE` and `psql` fell back to a local Unix socket. The repair maps the validated URI into explicit libpq fields, requires channel binding and passes a minimal read-only Production metadata probe. Gate 3C–3D still require the repaired clean exact commit before archive creation.
