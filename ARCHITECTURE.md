...
Formal Production is in the bounded V2 cutover maintenance window. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`. PF-001 is `High / Closed`; PF-002 and PF-003 are `Normal / Closed by explicit acceptance`. V2-8-3 Gate 0B and Gates 1–4 are complete. Gate 5 has rotated the old V1 database credential, created the final encrypted Schema 5 backup and no-compute recovery branch, and migrated Neon `main` to Schema Version 6 with full parity across all 1,854 legacy core rows. The exact Git `main` readiness/live deployment and runtime acceptance remain before the maintenance window closes. The whole V2 release excludes SSO（Single Sign-On，单点登录）and confidential per-person authorization.
...
The cloud-backed V1 chronology above remains historical evidence. Stage 1 through V2-8-2 built and rehearsed the complete V2; V2-8-2.1 stabilised runtime performance and V2-8-3 Gate 0B verified its exact commit on protected Preview in `SYD1`. V2-8-2.2 adds reliable keyboard rating selection, device-selectable fallback voices, paired English/Chinese examples, JSON backup Version 4 and additive Schema 6 migration `0004`. PF-001 is `High / Closed` with exact Standard-C playback, additive `0005`, local corpus evidence, keyless Preview-only WIF, exact deployment and MacBook + Chrome human acceptance. PF-002 and PF-003 are `Normal / Closed by explicit acceptance`. V2-8-3 Gates 0B–4 are complete; Gate 5 has placed Production in maintenance, blocked the retained V1 database credential, completed a final independently restorable backup and migrated `main` to Schema 6 with full parity. Production-only Gemini/TTS identities are configured but remain Kill-Switched until the exact `main` live acceptance.
