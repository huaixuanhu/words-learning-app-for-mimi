...
Formal private V1 is live on Neon `main` Schema Version 5. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`; the Stage 6B-P1-G-C-5 record is the current release source of truth. V2-8-3 now has a completed local Gate 1 protection layer and a completed Gate 0B exact-commit protected Preview verification. Gate 0B read Vercel/Neon Preview metadata only; it did not change Production, credentials, aliases, environment values, checkpoint state or Gemini usage. The chronological inventory below retains the pre-launch path that led to this state.
...
The root `vercel.json` pins V2 Vercel Functions to the single Sydney region `syd1`, beside Neon `ap-southeast-2`; static assets remain CDN-served. Separately approved Gate 0B deployment `dpl_3DeE8BrKcZ9cBdCkPA4jE1UTeHXf` now proves the exact performance commit and observed application/API Functions in `SYD1`. The fixed historical Preview alias was not repointed; the exact artifact is available through its protected unique and branch URLs. Schema Version 6, backup version 3, AI controls, accepted Motion/reduced-motion behavior, scheduling rules and Production topology are unchanged.
