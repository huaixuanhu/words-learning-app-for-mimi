...
- P1-G-C-3 adds `plan_docs/PLAN_V1_STAGE6B_P1_G_C_3_DASHBOARD_EVIDENCE.md` as the approved read-only Neon dashboard evidence capture. It confirms the current resource has only one `main` default branch, `neondb` / `neondb_owner`, no child branches, no visible distinct Production target, and a 6-hour restore window. P1-G-C now moves to target strategy selection.
