...
The cloud-backed V1 described above is live and remains the current Production behavior. On branch `V2`, Stage 1 completed the isolated daily-study contract, Stage 2 / 2-B completed the bounded Gemini quality evidence, Stage 3 completed the local/application Schema Version 6 plus backup version 3 data layer, Stage 3.1 completed the bounded local Review interaction/context-word-action pass, Stage 4 completed the mobile foundation and English-first copy pass, Stage 5 completed the local daily-learning engine under `plan_docs/PLAN_V2_STAGE5_DAILY_LEARNING_ENGINE.md`, Stage 5.1 completed the daily-episode scheduling repair under `plan_docs/PLAN_V2_STAGE5_1_DAILY_EPISODE_SCHEDULING_REPAIR.md`, Stage 6 completed the local Active Practice Engine under `plan_docs/PLAN_V2_STAGE6_ACTIVE_PRACTICE_ENGINE.md`, and Stage 7A completed the honest local-fixture AI experience and dormant formal safety boundary. `V2-7B-1` completed the server-owned formal route locally, and `V2-7B-2` then proved it through one disposable schema-only Neon child and two synthetic Gemini requests. V2-8-1 completed the compact Dashboard Actual/estimate layer and responsive acceptance; V2-8-1.1 then removed redundant, atmospheric, and developer-facing learner copy under a dedicated regression contract. Replay, Cache, the database cap, and Kill Switch added no provider request. Both temporary Auth Keys, the child branch/role, and local secret file were deleted. The live V1 database remains Schema Version 5; no Production migration, Production data write, Vercel change, or deployment is authorized. The next proposed product slice is separately approved V2-8-2.
...
- Formal `/api/ai/enrichment` and `/api/ai/context-explain` handlers re-check Basic Auth, same-origin, JSON body size, exact public request shape, runtime mode, and Disclosure Version. V2-7B-1 adds server-owned item lookup, session-bound current-disclosure evidence, canonical SHA-256 hashes, Cache and complete Idempotency, same-Cache in-flight protection, atomic quota reservation, source revalidation, transactional result persistence, and server mutations for accept/reject/`Add to learning`. V2-7B-2 adds an exact `v2-7b-2-local-smoke` scope that also requires loopback origin, `postgres-preview`, Schema 6/accounting readiness, a confirmed temporary target/Auth Key/logging state, no Vercel marker, and at most two database-counted provider attempts. The Gemini adapter stays lazy, server-only, pinned, tool-free, retry-free, sends top-level `store: false`, and is guarded by a 90-second timeout. With the temporary branch, key, and environment file deleted, the formal provider route has returned to its resting state.
