...
Formal private V1 is live on Neon `main` Schema Version 5. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`; the Stage 6B-P1-G-C-5 record is the current release source of truth. PF-001 is `High / Closed`: non-Production `staging` has `0004` and `0005`, protected Preview runs the exact Standard-C candidate through keyless Preview-only WIF, machine playback passed, and the user passed Settings, Recognition, Active and example-word playback on MacBook + Chrome with “非常理想” audio. Mimi later independently confirmed the audio had no problem without recording an exact device/browser; iPhone + Safari remains untested. V2-8-2.3 is active again for PF-002, now `Normal / Fixed locally`: the historical exact Preview confirmed Review/New Learning navigation, goal hierarchy, session labels and panel alignment, but its Instrument Serif typography was rejected by the user as uncomfortable. The local candidate restores Geist titles and the prior large-word fallback without changing learning data, scheduling, layout or Motion; fresh exact Preview and human retest remain pending. Production remains V1 on Schema 5. V2-8-3 Gate 2 is paused until PF-002 closes or is explicitly accepted and would still require fresh approval. The whole V2 release excludes SSO（Single Sign-On，单点登录）and confidential per-person authorization.
...
The cloud-backed V1 described above remains the current Production behavior. On branch `V2`, Stage 1 through V2-8-2 built and rehearsed the complete V2; V2-8-2.1 stabilised runtime performance and V2-8-3 Gate 0B verified its exact commit on protected Preview in `SYD1`. V2-8-2.2 adds reliable keyboard rating selection, device-selectable fallback voices, paired English/Chinese examples, JSON backup Version 4 and additive Schema 6 migration `0004`; its data migration is present on non-Production `staging`. PF-001 is `High / Closed` with exact Standard-C playback, additive `0005`, local corpus evidence, keyless Preview-only WIF, exact deployment and MacBook + Chrome human acceptance. PF-002 reopens V2-8-2.3 and is `Normal / Fixed locally` after the user rejected the historical Preview font and a bounded Geist rollback was implemented; V2-8-3 local Gate 1 and remote Gate 0B remain complete while Gate 2 is paused pending a fresh PF-002 Preview, human closure/acceptance and later fresh approval. Production V1, Neon `main`, Production secrets, and Production Schema 5 data remain unchanged; no V1 patch is planned.
...
V2-8-2.3 owns the release-quality process contract for protected Preview findings. PF-001 is `High / Closed`: required `staging` migrations, exact application refresh, Kill Switch, machine routes and MacBook + Chrome human retest passed; Mimi later confirmed the audio without an exact environment record. PF-002 is `Normal / Fixed locally`: desktop navigation uses `Review / New Learning`, mobile uses `Learn`, both Tracks expose accessible zone links, Home/Study use a two-goal hierarchy, and session summaries distinguish Daily goal, today’s distinct actual and Ready now. Historical exact commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131` runs as protected Preview deployment `dpl_DxQWHVspQuukaeR6LTCjKp15a3zp` in `SYD1` and passed machine checks, but the user rejected its Instrument Serif typography after real use. The current local candidate no longer loads Instrument Serif: display titles use Geist semibold and large vocabulary restores the earlier Georgia fallback, while component sizing, body/control/data fonts and ChillRoundF remain unchanged. A fresh exact Preview and human retest remain pending; Gate 2 remains paused.
...
`plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md` is the canonical Production backup, migration and cutover plan. Local Gate 1 and the separately approved protected Preview Gate 0B are complete:
...
- Non-empty Schema 5 inventory, Schema 5 → 6 migration, Schema 6 inspection, and full parity commands exist for a Production clone and `main`. They are dormant until exact target, database, role, action and human-confirmation guards agree with authenticated live GETs to Neon's fixed official control-plane API. The response must bind project, endpoint, branch, parent, state and clone source before any Postgres connection; local JSON/hash self-attestation is rejected. Gate 1 also leaves the code-owned approved Production-project SHA-256 as `null`, so no real command can reach the control plane or Postgres until Gate 2 independently confirms that project and a reviewed code change pins its digest. Clone mode explicitly refuses the live confirmed `main` endpoint; `main` migration additionally requires verified backup, recovery point, clone rehearsal, write-free evidence and mechanical blocking of the retained V1 runtime write path.
...
Formal private V1 is live from `main` behind application Basic Auth. Vercel Production uses `postgres-production` against Neon `main`; Development / Preview use Neon `staging` and must not promote non-production business rows. `plan_docs/PLAN_V1_STAGE6B_P1_G_C_5_LIVE_PRODUCTION_EXECUTION.md` is the current live release source of truth, and `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md` is the future V2 cutover source. Branch `V2` remains a development branch. Gate 0B remains complete; V2-8-2.3 is active for PF-002. Local cutover tooling and runtime gates still do not authorize V2 Production migration, pull request, merge, credential work or Production deployment; Gate 2 is paused and every remote gate requires separate Tier 3 approval.
