...
Formal private V1 is live on Neon `main` Schema Version 5. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`; the Stage 6B-P1-G-C-5 record is the current release source of truth. V2-8-2.2 is complete on the local V2 branch, while protected Preview / `staging` still run the prior exact deployment and data contract. V2-8-3 has a completed local Gate 1 protection layer and a completed Gate 0B exact-commit protected Preview verification. No V2-8-2.2 remote migration, provider call, credential change or deployment has occurred. The chronological inventory below retains the pre-launch path that led to this state.
...
The cloud-backed V1 described above remains the current Production behavior. On branch `V2`, Stage 1 through V2-8-2 built and rehearsed the complete V2; V2-8-2.1 stabilised runtime performance and V2-8-3 Gate 0B verified its exact commit on protected Preview in `SYD1`. V2-8-2.2 now adds reliable keyboard rating selection, device-selectable browser English voices, paired English/Chinese examples, JSON backup Version 4 and additive Schema 6 migration `0004`. This newest slice is local only. V2-8-3 local Gate 1 and remote Gate 0B are complete; Gate 2 is the next separately approved remote slice. Production V1, Neon `main`, Production secrets, and Production Schema 5 data remain unchanged; no V1 patch is planned.
...
Formal private V1 is live from `main` behind application Basic Auth. Vercel Production uses `postgres-production` against Neon `main`; Development / Preview use Neon `staging` and must not promote non-production business rows. `plan_docs/PLAN_V1_STAGE6B_P1_G_C_5_LIVE_PRODUCTION_EXECUTION.md` is the current live release source of truth, and `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md` is the future V2 cutover source. Branch `V2` remains a development branch. Gate 0B authorized only its exact protected Preview deployment and read-only checks and is complete. Local cutover tooling and runtime gates still do not authorize V2 Production migration, pull request, merge, credential work or Production deployment; Gate 2 and every later remote gate require separate Tier 3 approval.
