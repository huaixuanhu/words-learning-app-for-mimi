...
Formal private V1 is live on Neon `main` Schema Version 5. Current operational tier, target capability tier, and working tier are Tier 3 under `human-ai-governance v0.3.0`; the Stage 6B-P1-G-C-5 record is the current release source of truth. V2-8-2.2 is complete on the local V2 branch, while protected Preview / `staging` still run the prior exact deployment and data contract. V2-8-2.3 is now the active documentation-defined Preview feedback stabilisation stage. V2-8-3 has a completed local Gate 1 protection layer and a completed Gate 0B exact-commit protected Preview verification, while Gate 2 is paused until V2-8-2.3 closes and will still require fresh approval. No V2-8-2.2 or V2-8-2.3 remote migration, provider call, credential change or deployment has occurred. The whole V2 release excludes SSO（Single Sign-On，单点登录）and confidential per-person authorization. The chronological inventory below retains the pre-launch path that led to this state.
...
The cloud-backed V1 described above remains the current Production behavior. On branch `V2`, Stage 1 through V2-8-2 built and rehearsed the complete V2; V2-8-2.1 stabilised runtime performance and V2-8-3 Gate 0B verified its exact commit on protected Preview in `SYD1`. V2-8-2.2 adds reliable keyboard rating selection, device-selectable browser English voices, paired English/Chinese examples, JSON backup Version 4 and additive Schema 6 migration `0004`; this slice is local only. V2-8-2.3 now owns the numbered pre-launch Preview feedback register and bounded fix/retest loop. V2-8-3 local Gate 1 and remote Gate 0B remain complete, while Gate 2 waits for V2-8-2.3 closure and a later fresh approval. Production V1, Neon `main`, Production secrets, and Production Schema 5 data remain unchanged; no V1 patch is planned.
...
V2-8-2.2 centralizes Review shortcut eligibility and 2×2 rating movement for Recognition and Active. Buttons such as the card, answer toggle, and pronunciation no longer suppress Arrow selection; text entry, dialogs, IME composition and modified shortcuts remain excluded. Settings reads only the browser-provided English `SpeechSynthesisVoice` list, ranks a stable `Best available`, lets the learner preview and save a device-local choice, and falls back when that voice disappears. No audio or learning evidence leaves the browser. Cloud TTS is a separate future gate if real-device listening remains unsatisfactory.
...
V2-8-2.3 adds no runtime component or data shape. It is the release-quality process contract for protected Preview findings: each accepted issue receives a `PF` identifier, reproduction evidence, priority, bounded local fix, exact-commit Preview retest and human closure result. The current Preview must receive required migrations before its application refresh. Every `Blocker` and `High` item must close before Gate 2 can be reconsidered; this stage does not itself authorize a migration, deployment, credential change, provider call or Production action.
...
V2-3 established the persisted boundary as Schema Version 6 and JSON backup Version 3. V2-8-2.2 keeps that schema name, adds forward-only `0004_v2_bilingual_examples.sql`, and advances JSON backup to Version 4 for aligned Chinese example translations. Versions 1–3 remain readable and preserve missing translations as explicit blanks. Existing `0001` / `0002` / `0003` migrations remain immutable. `0003` was executed on the deleted proof/rehearsal children and long-lived non-Production `staging`; `0004` has not been executed on any remote target. Production `main` remains Schema Version 5.
...
- `db/migrations/0004_v2_bilingual_examples.sql` is independently pinned to SHA-256 `9e00e1213366492db90a97709d605d68770a1b80f46caa8b748886bc8bd4e29c`. The forward sequence is `0003` then `0004` in one transaction; partial readiness is rejected.
...
Formal private V1 is live from `main` behind application Basic Auth. Vercel Production uses `postgres-production` against Neon `main`; Development / Preview use Neon `staging` and must not promote non-production business rows. `plan_docs/PLAN_V1_STAGE6B_P1_G_C_5_LIVE_PRODUCTION_EXECUTION.md` is the current live release source of truth, and `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md` is the future V2 cutover source. Branch `V2` remains a development branch. Gate 0B authorized only its exact protected Preview deployment and read-only checks and is complete. V2-8-2.3 now owns pre-launch feedback closure. Local cutover tooling and runtime gates still do not authorize V2 Production migration, pull request, merge, credential work or Production deployment; Gate 2 is paused until V2-8-2.3 closes and every later remote gate requires separate Tier 3 approval.
