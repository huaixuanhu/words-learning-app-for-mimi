...
- Latest Stage 6B-P1-G-C-5 update: `plan_docs/PLAN_V1_STAGE6B_P1_G_C_5_LIVE_PRODUCTION_EXECUTION.md` records the completed formal V1 launch. Neon has clean schema-version-5 `main` and `staging` branches; `staging` is the non-production/default branch, Development / Preview use `staging`, Production uses `main`, and credentials are distinct. The Marketplace project connection is removed, minimal variables are scoped manually, and per-feature Preview branch automation is deferred. Application-level Production Basic Auth protects the current private trusted group, while `person_id` remains data separation rather than authorization. PR `#1` merged `V1` to `main`; final functional deployment `dpl_ZrEHc39z4dufgq3RcPQGUV1vK4n2` is Ready on the canonical domain, reports `postgres-production`, returns schema version 5 with empty formal data, and has no recorded error/5xx logs. The first real user action remains the first formal write acceptance.
