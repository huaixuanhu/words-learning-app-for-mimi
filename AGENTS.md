...
- Current branch: `V2`.
- Current live product: the canonical Production domain is in the bounded V2 cutover maintenance window behind Basic Auth（基础认证）; Neon `main` is Schema Version 6 after verified migration/parity, while the exact Git `main` application deployment is pending.
...
- Canonical V2 Stage 8-3 child plan: `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md`. Gate 0B and Gates 1–4 are complete. The user explicitly authorized execution through full V2 launch; Gate 5 has completed Production identities, maintenance, old-credential revocation, final backup, Schema 5 recovery point, `main` migration and full parity. Exact Git `main` readiness/live deployment and runtime acceptance remain.
...
- V2 status: Stage 1 through V2-8-2.3 are complete. PF-001 is `High / Closed`; PF-002 and PF-003 are `Normal / Closed by explicit acceptance`. V2-8-3 Gate 0B and Gates 1–4 are complete. Gate 5 has moved Production into maintenance, blocked the old V1 database credential, retained two independent Schema 5 recovery paths and migrated Neon `main` to Schema Version 6 with full parity. AI/TTS remain Kill-Switched pending exact Git `main` live acceptance. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
