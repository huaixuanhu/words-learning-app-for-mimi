...
- Canonical active PF-001 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_1_GOOGLE_CLOUD_STANDARD_TTS.md`; Gate B local implementation, bounded Gate C user-ADC proof and Gate D local human acceptance are complete. Gate E is approved and has completed Preview-only WIF plus `staging` `0004` / `0005`; exact Preview deployment and human retest remain open. The exact Voice Contract is `en-AU-Standard-C`, speaking rate `0.9`, pitch `0`, MP3. The accepted evidence records 46 Good, 2 Review, 0 Bad and 2 unrecorded samples.
