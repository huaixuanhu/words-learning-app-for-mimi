...
- Current application stack: Next.js App Router, TypeScript, Tailwind CSS, ESLint, Vitest, npm, `@neondatabase/serverless` for approved database scripts, `dotenv-cli` for explicit local env loading, `ts-fsrs` for isolated Stage 8-B Recognition FSRS calibration, and browser `localStorage` for current app study data.
