...
- Current stage: Stage 5I runtime Postgres adapter implementation. Browser-local storage schema version 3 still drives the user-facing app runtime. The Vercel project is linked, a Neon Postgres development / preview resource exists, and `db/migrations/0001_initial.sql` has been applied to the non-production development database with an empty schema verification. Vercel currently has an active Production deployment (`dpl_2nvALJ1CutPjeFteXKMCHWKa4UsD`) from branch `V1`; it is documented as a non-official artifact and should not be treated as the formal V1 production release. Verified Preview deployments exist for manual CLI preview and clean Git integration preview. Stage 5I adds a server-only development / preview Postgres runtime adapter plus disabled-by-default health/smoke routes, but no production database migration, production import, production runtime cutover, backup import, or user-facing storage cutover has been implemented yet.
