...
- Canonical active PF-001 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_1_GOOGLE_CLOUD_STANDARD_TTS.md`; Gate B local implementation, bounded Gate C user-ADC proof and Gate D local human acceptance are complete. The exact local Voice Contract is `en-AU-Standard-C`, speaking rate `0.9`, pitch `0`, MP3. The accepted evidence records 46 Good, 2 Review, 0 Bad and 2 unrecorded samples; all Preview/Production work remains open.
...
- V2 status: Stage 1 through V2-8-2 are complete within their approved scopes. V2-8-2.1 is separately proven on protected Preview; V2-8-2.2 is complete on the local branch only; V2-8-2.3 is active. PF-001 is `High / Preview-ready`: the local candidate has strict `/api/tts`, explicit device fallback, fixture and Google adapters, Runtime Cache adapter, independent accounting and additive `0005`; user audition selected `en-AU-Standard-C`. Local human listening accepts the voice with two minor stress observations and two unrecorded samples retained as an evidence limitation; protected Preview migration/deployment/retest remains open. Long-lived `staging` / protected Preview still run the prior contract; Production V1 and Neon `main` remain Schema Version 5. V2-8-3 Gate 2 stays paused. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
