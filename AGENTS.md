...
- Current local branch: `main`. Local `main`, `origin/main` and `origin/v2.1` reached the V2.1 application release commit `c3901cbf5e51f6b4e28700b3c0507cf31fe6c76d` before the documentation closeout.
- Current live product: V2.1 is live on the canonical Production domain behind Basic Auth（基础认证）. Vercel Production runs the exact `main` application in `syd1`; Neon `main` is Schema Version 6 with the V2.1 vocabulary uniqueness index; Gemini enrichment and Google Cloud Standard-C TTS are enabled under their independent Production guards.
...
- Canonical V2 Stage 8-3 child plan: `plan_docs/PLAN_V2_STAGE8_3_PRODUCTION_BACKUP_MIGRATION_CUTOVER.md`. V2 launch through Gate 6 is complete: Production identities, maintenance, old-credential revocation, Schema 5 backup/recovery, `main` migration, full parity, exact `main` live deployment, real Gemini/TTS smoke and steady-state accounting all passed. Gate 7 post-launch observation remains open until one Australia/Melbourne natural-day boundary and real learning runs from both users can be recorded honestly.
...
- Canonical completed V2-8-3 Gate 3 child plan: `plan_docs/PLAN_V2_STAGE8_3_GATE3_ENCRYPTED_LOGICAL_BACKUP_RESTORE_REHEARSAL.md`. Exact commit `88ddb1c2c96437ca3cc4a8f2103990ae70e79eac` has a repository-external age-encrypted Production Schema 5 archive and a successful isolated PostgreSQL 17 restore. Source/restore counts, invariants, table digests and combined digest match; evidence, permissions, Keychain custody, clipboard clearing and cleanup pass.
- Canonical completed local V2.1 repair plan: `plan_docs/PLAN_V2_1_DUPLICATE_IMPORT_DEDUPLICATION.md`. It prevents duplicate manual add/edit/import writes, adds selected-person confirmation-gated cleanup and includes a fail-closed `0006` unique-index migration. Its original local completion granted no Production authority; the later Production release used a separate accepted gate.
- Canonical completed V2.1 Production gate: `plan_docs/PLAN_V2_1_PRODUCTION_DUPLICATE_REPAIR_RELEASE.md`. Preview acceptance, Staging rehearsal, pre-change encrypted backup/restore, exact `main` deployment, separately confirmed real cleanup, Production `0006`, browser/runtime checks and post-change encrypted backup/restore all pass.
- V2 status: fully live in Production. Stage 1 through V2-8-2.3 and V2-8-3 Gates 0B–6 are complete. PF-001 is `High / Closed`; PF-002 and PF-003 are `Normal / Closed by explicit acceptance`. Production retains the prior V1 artifact, a no-compute Schema 5 recovery branch, pre-cutover and post-cutover encrypted backups, and a forward-repair rollback decision. Gate 7 stability observation and later resource cleanup remain operational follow-up; they do not mean V1 is still serving the product. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
- V2.1 is complete and live. Its duplicate identity is `(person_id, normalized_text)`; cleanup keeps one deterministic item, does not merge independent FSRS histories, preserves creation facts/import-batch audit rows and hides zero-remaining batches from normal Library history. Production cleanup removed 1,204 duplicate copies from 75 groups and left 282 unique items, 125 review states, 203 review events, all 38 import-batch audit rows and zero checked invariant failures. `0006` replaced the old non-unique index with the unique index.
...
- V2-8-2.2 is complete. It restores Arrow selection after non-input learning buttons receive focus, shares one guarded 2×2 keyboard contract across Recognition/Active, adds a device-local browser English voice selector and preview, and carries aligned Chinese example translations through learner UI, import/edit, AI acceptance, Postgres/local storage, CSV and JSON backup Version 4. `0004_v2_bilingual_examples.sql` is applied to both non-Production `staging` and Production `main`. Accepted Motion/reduced-motion and scheduling remain unchanged.
...
- PF-001 implements an independent TTS guard: global 2,000 provider attempts/day, 100,000 characters/day, 1,000,000 characters/month, list-price-equivalent US$0.50/day and US$4/month, concurrency 4, Cache and Kill Switch, with no per-person limit. Cache hits do not count as provider attempts. `0005` is applied to `staging` and Production `main`. Production uses its own WIF identity; one live Standard-C miss and identical Cache hit passed with zero in-flight calls.
- V2-8-3 Gate 1 added exact `maintenance` / `schema6-readiness` / `live` cutover modes, old-client mutation refusal, non-empty Schema 5/6 clone/main tooling, separately pinned `0003` / `0004` / `0005` migration SHA-256 values, secret-free cutover/paired-rollback evidence, and Production-only provider scopes. Production now runs `live`; missing or unknown modes still fail closed, Basic Auth remains ahead of application routes, and mutations require the `v2-schema6` client marker.
...
- Current topology: Neon `main` is Production Schema 6; Neon `staging` is the Development / protected V2 Preview baseline at Schema 6. A no-compute Production Schema 5 recovery branch, the Gate 4 clone/preserved state and the earlier Staging checkpoint are retained through stability closeout. Production and non-production credentials are distinct; Production runtime is `postgres-production`.
- Branch `main` and the historical `V2` Preview carry the code-owned Vercel `syd1` region contract. The canonical Production deployment and protected Preview application Functions are in `SYD1`.
...
- V2 includes the first formal AI enrichment path for extra meanings, examples, similar words, and confusable words, with editable preview and explicit human acceptance.
...
- Current Production AI is paid `gemini-3.1-flash-lite`; the fixed 120-entry evaluation and Production provider smoke have passed.
...
- The approved Stage 2 test key may exist only in ignored `.env.stage2.local`; never print, hash, stage, commit, copy to `.env.local`, or upload it to Vercel. Production uses a separately created, API-restricted credential.
...
- The Stage 2 fixture evaluation and V2-7B-2 disposable provider proof used versioned provider/data/cost boundaries. Production calls continue to require the current in-product disclosure, restricted Production credential, strict route, atomic accounting and explicit human acceptance before saved learning data changes.
- V2-3 established Schema Version 6 and JSON backup Version 3. V2-8-2.2 adds `0004_v2_bilingual_examples.sql` and JSON backup Version 4; PF-001 adds operational-only `0005_v2_standard_tts_accounting.sql` without changing the Schema Version name or backup shape. `0003`–`0005` are applied to Production `main`.
...
- Production AI uses the independent `v2-8-3-production` scope. It requires `live`, HTTPS, Vercel Production `main`, `NODE_ENV=production`, `postgres-production`, Schema 6, atomic accounting, all Production confirmations and explicit Kill Switch state. The initial four-attempt ceiling passed its bounded smoke and was removed through recorded steady-state acceptance; the long-term 300-attempt/day, token, cost, concurrency, Cache, Idempotency and Kill Switch guards remain.
...
V2-8-3 Gate 3 completion supersedes only the suite count above: exact commit `88ddb1c2c96437ca3cc4a8f2103990ae70e79eac` passes PostgreSQL 17.10 and `age` 1.3.1 tool checks plus same-commit cross-timezone synthetic backup/restore, wrong-identity rejection, corruption rejection and cleanup. The real Production `main` run consumed and cleared one Neon Console unpooled connection string, verified the pinned endpoint and Schema 5 baseline, streamed `pg_dump` through `age`, and restored locally through a Unix socket. Source/restore counts and eight invariants match; combined SHA-256 is `6c82ba44caf462051b9579ca90f8a59994c8794649d6e871dd135cf0c23e2aa9`. The `112,253`-byte archive SHA-256 is `d30950ee9aecefb2863ad6143494707dd723d8e89a50117bc0cf4b46662847a0`; directory/file permissions are `0700/0600`, evidence secret scan passes, clipboard is empty and no temporary Gate 3 directory remains. Final validation passes 92 files / 566 tests with the existing Postgres integration file/test skipped, lint, typecheck, all three application backup dry-runs, Production build, Tier 3 governance and diff checks. Gate 3 is complete at Approval Stop 3; Gate 4 remains unapproved.

V2-8-3 Production release supersedes the operational state above. Production `main` completed pinned `0003 -> 0004 -> 0005` with 1,854-row core parity, and exact V2 is live in `syd1`. Real Production acceptance used 2 successful Gemini attempts / 1,440 tokens / `US$0.000887` and 1 successful TTS attempt / 8 characters / `US$0.000032`; Replay/Cache added no provider attempt and both ledgers returned to zero in-flight. The AI draft was rejected without vocabulary mutation. The live cutover manifest validates, and the immediate 222,110-byte Schema 6 encrypted backup restored into isolated PostgreSQL 17 with matching Schema/count/invariant snapshot. Security closeout uses Next.js 16.2.11 and Sharp 0.35.3; full Vitest passes 92 files / 567 tests with the existing Postgres integration file/test skipped, lint, typecheck, all three backup dry-runs, Production build, manifest, Tier 3 governance and full npm audit pass. Protected Preview, V1 artifact and Schema 5 recovery remain until Gate 7 records one natural-day boundary plus user and Mimi learning evidence.

V2.1 Production release supersedes the local-only snapshot above. Full Vitest passes 96 files / 589 tests with the existing Postgres integration file/test skipped. Added coverage owns deterministic keeper selection, selected-person isolation, review/AI hard-delete propagation, stale confirmation refusal, duplicate-only/same-batch/repeated import refusal, manual add/edit collisions, Postgres transaction revalidation, UI confirmation, source-pinned Production/Staging guards, binary-safe encrypted backup/isolated restore and fail-closed `0006`. ESLint, TypeScript, all three backup dry-runs, Production build, dependency audit, governance and diff checks pass. Production `0006` is applied after zero-duplicate verification; the post-change 180,803-byte `age` archive restores successfully with matching counts, invariants and table digests.
