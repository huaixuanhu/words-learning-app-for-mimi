...
- Current stage: Stage 6A Production（生产环境）release gate design is complete after Stage 5N Preview（预览环境）UI（用户界面）runtime verification. Browser-local storage schema version 3 remains the default user-facing fallback, while the Vercel project is linked, a Neon Postgres development / preview resource exists, and `db/migrations/0001_initial.sql` has been applied only to the non-production development database. Stage 6A confirms the release sequence as Stage 6A gate design, then Stage 7 UI / visual design and optional PWA（Progressive Web App，渐进式 Web 应用）work, then Stage 6B merge（合并）to `main` and formal Production execution after explicit approval. The Stage 6A gate also records that `person_id` separates learner data but is not security isolation, `postgres-preview` must not be used as a Production runtime, and durable Production writes require either explicit no-credential private-URL risk acceptance or a separate access gate. The existing active Production deployment (`dpl_2nvALJ1CutPjeFteXKMCHWKa4UsD`) from branch `V1` remains documented as a non-official artifact and must not be treated as the formal V1 production release. No production database migration, production import, production runtime cutover, formal user backup import, or formal production deployment has been implemented yet.
