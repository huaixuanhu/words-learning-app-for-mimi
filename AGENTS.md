...
- Canonical active V2-8-3 Gate 3 child plan: `plan_docs/PLAN_V2_STAGE8_3_GATE3_ENCRYPTED_LOGICAL_BACKUP_RESTORE_REHEARSAL.md`. Gate 3A–3B local implementation and synthetic proof pass. Gate 3C safely exposed and repaired the URI-to-libpq mapping, then a second clean-commit attempt reached isolated restore and rejected cross-timezone `timestamptz` digest differences. Inventory now canonicalizes row serialization to UTC; Melbourne-source/UTC-restore synthetic parity passes and waits for a new clean exact commit before retrying Production.
- V2 status: Stage 1 through V2-8-2.3 are complete within their approved scopes. PF-001 is `High / Closed`; PF-002 and PF-003 are `Normal / Closed by explicit acceptance`. V2-8-3 Gate 0B, local Gate 1 and remote read-only Gate 2 are complete; Gate 3 is approved and in progress at its UTC digest repair clean-commit checkpoint. Production V1 and Neon `main` remain Schema Version 5. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
...
V2-8-3 Gate 3 UTC-digest checkpoint supersedes only the suite count above: focused backup contracts pass 1 file / 12 tests and the full suite passes 92 files / 566 tests with the existing Postgres integration file/test skipped. PostgreSQL 17.10 plus `age` 1.3.1 tool checks, cross-timezone synthetic encrypted backup/restore, wrong-identity and corruption rejection, cleanup, lint, typecheck, all three application backup dry-runs and Production build pass; Tier 3 governance and diff checks follow synchronized checkpoint records. Vercel Sensitive values are non-readable; the Production runner consumes one Neon Console `main` unpooled connection string from the system clipboard, clears it before parsing/connection and verifies the code-pinned endpoint SHA-256. Gate 3C Attempt 1 exposed and repaired the URI/libpq mapping. Attempt 2 reached the isolated restore and then rejected all non-empty table digests because identical `timestamptz` instants were rendered in different server timezones; counts were not listed as mismatches, both empty tables matched, and the archive/cluster/clipboard were cleaned. Inventory now sets local UTC before JSON serialization, and Gate 3C–3D wait for this repair to be committed and re-proved against the new clean exact commit.
