...
- Current stage: Stage 5N Preview（预览环境）UI（用户界面）runtime verification is complete. Browser-local storage schema version 3 remains the default user-facing fallback, while the Vercel project is linked, a Neon Postgres development / preview resource exists, and `db/migrations/0001_initial.sql` has been applied only to the non-production development database. Stage 5N-A verified a read-only Preview deployment, and Stage 5N-B temporarily enabled Preview UI writes, wrote one controlled smoke row through `/api/storage/data`, cleaned that row set, removed the write flag, redeployed a disabled Preview, and removed the temporary write-enabled deployment. The development database currently has zero rows in core study tables. The confirmed release sequence is Stage 6A Production（生产环境）release gate design, then Stage 7 UI / visual design and optional PWA（Progressive Web App，渐进式 Web 应用）work, then Stage 6B merge（合并）to `main` and formal Production execution after explicit approval. The existing active Production deployment (`dpl_2nvALJ1CutPjeFteXKMCHWKa4UsD`) from branch `V1` remains documented as a non-official artifact and must not be treated as the formal V1 production release. No production database migration, production import, production runtime cutover, formal user backup import, or formal production deployment has been implemented yet.
