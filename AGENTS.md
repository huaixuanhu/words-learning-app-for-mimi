...
- V2 status: Stage 1 through V2-8-2 are complete within their approved scopes. V2-8-2.1 is separately proven on protected Preview; V2-8-2.2 is complete on the local branch only; V2-8-2.3 is the active documentation-defined Preview feedback stabilisation stage. The current local contract is Schema Version 6 plus additive `0004`, JSON backup Version 4, shared Recognition/Active Arrow controls, device-selectable browser English voice, and aligned bilingual examples. Long-lived `staging` / protected Preview still run the previous Schema 6 contract; Production V1 and Neon `main` remain Schema Version 5. V2-8-3 local Gate 1 and remote Gate 0B are complete; Gate 2 is paused until V2-8-2.3 closes and still requires fresh approval. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
...
- V2-8-2.3 is documentation-only at creation. It uses numbered `PF` records, priority/status rules, bounded local fixes, exact-commit Preview retest and human closure. Every `Blocker` and `High` item must close before V2-8-3 Gate 2 can be reconsidered. This plan itself authorizes no remote action.
