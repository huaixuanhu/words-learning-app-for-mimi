...
- Canonical completed V2 Stage 8-2.3 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; it owns the pre-launch protected Preview issue register and closure evidence.
- Canonical completed PF-001 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_1_GOOGLE_CLOUD_STANDARD_TTS.md`; PF-001 is `High / Closed`. Gate B–E implementation/provider/deployment evidence and Gate F human closure are complete. The exact Voice Contract is `en-AU-Standard-C`, speaking rate `0.9`, pitch `0`, MP3. Local corpus evidence records 46 Good, 2 Review, 0 Bad and 2 unrecorded samples; the final MacBook + Chrome Preview retest passed Settings, Recognition, Active and example-word playback with “非常理想” audio. Independent Mimi and iPhone + Safari results are not claimed.
...
- V2 status: Stage 1 through V2-8-2.3 are complete within their approved scopes. PF-001 is `High / Closed`; exact Standard-C Preview deployment and MacBook + Chrome human playback passed. Missing independent Mimi and iPhone + Safari evidence is explicit and not presented as passing compatibility. Production V1 and Neon `main` remain Schema Version 5. V2-8-3 Gate 2 is now the next candidate stage but still requires fresh approval. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
...
- V2-8-2.3 used numbered `PF` records, priority/status rules, bounded local fixes, exact-commit Preview retest and human closure. Its only registered item, PF-001, is closed; Gate 2 may now be considered only through fresh approval.
