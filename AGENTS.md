...
- Canonical active PF-002 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`; PF-002 is `Normal / Preview-ready`. It owns the Review/New Learning information architecture, two-goal hierarchy, session summary labels, Home panel alignment and title-only Instrument Serif contract. Exact protected Preview and machine acceptance are complete; human retest remains pending.
...
- V2 status: Stage 1 through the historical PF-001 V2-8-2.3 closeout are complete within their approved scopes. PF-001 is `High / Closed`; PF-002 is `Normal / Preview-ready`, so V2-8-2.3 remains active pending human retest. Production V1 and Neon `main` remain Schema Version 5. V2-8-3 Gate 2 is paused until PF-002 closes or is explicitly accepted, and would still require fresh approval. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
...
- V2-8-2.3 uses numbered `PF` records, priority/status rules, bounded local fixes, exact-commit Preview retest and human closure. PF-001 is closed. PF-002 exact commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131` is Ready as protected Preview deployment `dpl_DxQWHVspQuukaeR6LTCjKp15a3zp`; machine acceptance passed, human retest remains pending and Gate 2 remains paused.
...
PF-002 local validation supersedes the suite count above: the full suite passes 91 files / 553 tests with the existing Postgres integration file/test skipped. Focused learner-UI contracts pass 6 files / 24 tests. Lint, typecheck, all three backup dry-runs, Production build, governance and diff checks pass. Local Production-build browser checks pass at 1280 px and 390 px with no overlay or horizontal overflow; the two desktop Home panels both measure 488 px high, and Home/Study/Recognition/Active show the accepted Review/New Learning, goal and session-label hierarchy. Instrument Serif is limited to Latin 400-weight display titles and large words; Geist remains on body, controls, labels and numbers. Exact protected Preview commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131` repeats the 1280/390 px acceptance, both desktop panels at `488.32 px`, correct typography/control ownership, zero browser warning/error and zero Vercel Warning/Error/Fatal entries for the acceptance window. PF-002 is `Normal / Preview-ready` until human retest closes or explicitly accepts it.
