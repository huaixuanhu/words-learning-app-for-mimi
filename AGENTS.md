...
- V2.1 is complete only as a local branch candidate. Its duplicate identity is `(person_id, normalized_text)`; cleanup keeps one deterministic item, does not merge independent FSRS histories, preserves creation facts/import-batch audit rows and hides zero-remaining batches from normal Library history. Production remains unchanged until a separately approved backup/inventory/cleanup/migration/deployment gate.
