...
- Current local branch: `v2.1`, based on `main` commit `b6cf50f3422a69dbea9f59e998677f3c0bc49e8e`. Production still serves `main`.
- Current live product: V2 is live on the canonical Production domain behind Basic Auth（基础认证）. Vercel Production runs the exact `main` application in `syd1`; Neon `main` is Schema Version 6; Gemini enrichment and Google Cloud Standard-C TTS are enabled under their independent Production guards.
...
- Canonical accepted local V2.1 repair plan: `plan_docs/PLAN_V2_1_DUPLICATE_IMPORT_DEDUPLICATION.md`. It prevents duplicate manual add/edit/import writes, adds selected-person confirmation-gated cleanup and includes a fail-closed `0006` unique-index migration. No Production cleanup, migration or deployment is authorized by its local completion.
- Canonical accepted V2.1 Production gate: `plan_docs/PLAN_V2_1_PRODUCTION_DUPLICATE_REPAIR_RELEASE.md`. Preview human acceptance and Staging `0006` rehearsal/fixture cleanup pass. Production read-only inventory reports 75 duplicate identities and 1,204 removable copies among 1,486 items; Production remains unchanged until the pre-change encrypted backup, exact deployment and human cleanup-count stop pass.
...
- V2.1 application behavior is complete and Preview-accepted. Its duplicate identity is `(person_id, normalized_text)`; cleanup keeps one deterministic item, does not merge independent FSRS histories, preserves creation facts/import-batch audit rows and hides zero-remaining batches from normal Library history. The Production gate is approved but still stops before real cleanup until exact destructive counts are accepted.
...
V2.1 local validation supersedes only the local suite count: full Vitest passes 95 files / 583 tests with the existing Postgres integration file/test skipped. Added coverage owns deterministic keeper selection, selected-person isolation, review/AI hard-delete propagation, stale confirmation refusal, duplicate-only/same-batch/repeated import refusal, manual add/edit collisions, Postgres transaction revalidation, UI confirmation and fail-closed `0006`. ESLint, TypeScript, all three backup dry-runs and Production build pass; `0006` has not run remotely.
