...
- Canonical active V2-8-3 Gate 3 child plan: `plan_docs/PLAN_V2_STAGE8_3_GATE3_ENCRYPTED_LOGICAL_BACKUP_RESTORE_REHEARSAL.md`. Gate 3A–3B local implementation and synthetic proof pass. The first clean-commit Gate 3C runner stopped before archive creation because the URI-to-libpq environment mapping fell back to a local socket; the repaired explicit mapping passes a minimal read-only Production metadata query and now waits for a new clean exact commit before backup/restore.
...
V2-8-3 Gate 3 connection-repair checkpoint supersedes only the suite count above: focused backup contracts pass 1 file / 11 tests and the full suite passes 92 files / 565 tests with the existing Postgres integration file/test skipped. PostgreSQL 17.10 plus `age` 1.3.1 tool checks, synthetic encrypted backup/restore, wrong-identity and corruption rejection, cleanup, lint, typecheck, all three application backup dry-runs, Production build, Tier 3 governance and diff checks pass. Vercel Sensitive values are non-readable; the Production runner instead consumes one Neon Console `main` unpooled connection string from the system clipboard, clears it before parsing/connection and verifies the code-pinned endpoint SHA-256. Gate 3C Attempt 1 revealed that assigning the URI directly to `PGDATABASE` made `psql` use a local socket; explicit `PGHOST`/`PGPORT`/`PGDATABASE`/`PGUSER`/`PGPASSWORD`/TLS/channel-binding mapping now passes focused coverage and a minimal Production metadata query. Gate 3C–3D wait for this repair to be committed and its synthetic proof regenerated against the new clean exact commit.
