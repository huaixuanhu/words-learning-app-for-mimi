...
- Canonical active V2 Stage 8-2.3 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; it owns the pre-launch protected Preview issue register and closure gate.
- Canonical active PF-001 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_1_GOOGLE_CLOUD_STANDARD_TTS.md`; Gate B local implementation and bounded Gate C user-ADC proof are complete. The exact local Voice Contract is `en-AU-Standard-C`, speaking rate `0.9`, pitch `0`, MP3. Preview/Production work remains open.
...
- V2 status: Stage 1 through V2-8-2 are complete within their approved scopes. V2-8-2.1 is separately proven on protected Preview; V2-8-2.2 is complete on the local branch only; V2-8-2.3 is active. PF-001 is `High / Fixed locally`: the local candidate has strict `/api/tts`, explicit device fallback, fixture and Google adapters, Runtime Cache adapter, independent accounting and additive `0005`; user audition selected `en-AU-Standard-C`. The 50-entry corpus and protected Preview migration/deployment/retest remain open. Long-lived `staging` / protected Preview still run the prior contract; Production V1 and Neon `main` remain Schema Version 5. V2-8-3 Gate 2 stays paused. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
...
- V2-8-2.3 uses numbered `PF` records, priority/status rules, bounded local fixes, exact-commit Preview retest and human closure. Every `Blocker` and `High` item must close before V2-8-3 Gate 2 can be reconsidered.
- PF-001 implements an independent TTS guard in local code: global 2,000 provider attempts/day, 100,000 characters/day, 1,000,000 characters/month, list-price-equivalent US$0.50/day and US$4/month, concurrency 4, Cache and Kill Switch, with no per-person limit. Cache hits do not count as provider attempts. It has not been migrated or deployed remotely.
- V2-8-3 Gate 1 has locally added exact `maintenance` / `schema6-readiness` / `live` cutover modes, old-client mutation refusal, dormant non-empty Schema 5/6 clone/main tooling, separately pinned `0003` / `0004` / `0005` migration SHA-256 values, secret-free cutover/paired-rollback evidence, and Production-only provider scopes. Missing or unknown modes fail closed; Basic Auth remains ahead of maintenance; only `live` accepts Production mutations with the `v2-schema6` client marker. No remote database migration or deployment occurred. The live V1 and Neon `main` remain Schema Version 5.
...
- V2-3 established Schema Version 6 and JSON backup Version 3. V2-8-2.2 adds `0004_v2_bilingual_examples.sql` and JSON backup Version 4; PF-001 adds operational-only `0005_v2_standard_tts_accounting.sql` without changing the Schema Version name or backup shape. `0004` and `0005` have not run remotely, and Production `main` remains Schema Version 5.
...
PF-001 local validation supersedes that snapshot: 10 TTS/interaction focused files / 35 tests and 88 full-suite files / 536 tests pass, with the existing Postgres integration file/test skipped. It adds fixed Standard-C contract, same-origin route, explicit device fallback, fixture/Google adapters, Cache/coalescing, local/Postgres accounting, `0005`, migration-order guards and provider-data minimization. Lint, typecheck, all three backup dry-runs, script syntax, manifest template, Production build, production dependency audit, governance and diff checks pass. User selection of C is valid candidate evidence; the 50-entry corpus and protected Preview listening are still required before PF-001 closure.
