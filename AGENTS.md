...
- Canonical active PF-002 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`; PF-002 is `Normal / Fixed locally`. It owns the Review/New Learning information architecture, two-goal hierarchy, session summary labels, Home panel alignment and font-only rollback. The historical exact protected Preview passed machine acceptance, but the user found Instrument Serif uncomfortable in real use; titles now return to Geist and large vocabulary restores its prior Georgia fallback. A fresh exact protected Preview and human retest remain pending.
...
- V2 status: Stage 1 through the historical PF-001 V2-8-2.3 closeout are complete within their approved scopes. PF-001 is `High / Closed`; PF-002 is `Normal / Fixed locally`, so V2-8-2.3 remains active pending a fresh exact Preview and human retest. Production V1 and Neon `main` remain Schema Version 5. V2-8-3 Gate 2 is paused until PF-002 closes or is explicitly accepted, and would still require fresh approval. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
...
- V2-8-2.3 uses numbered `PF` records, priority/status rules, bounded local fixes, exact-commit Preview retest and human closure. PF-001 is closed. PF-002 historical exact commit `52942b412d0a2281c4dded2f5b9bd716d7fd0131` is Ready as protected Preview deployment `dpl_DxQWHVspQuukaeR6LTCjKp15a3zp`; machine acceptance passed, but the user rejected its Instrument Serif typography. The local Geist rollback is not yet deployed, human retest remains pending and Gate 2 remains paused.
