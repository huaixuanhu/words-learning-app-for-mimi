...
- Canonical active PF-002 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`; PF-002 is `Normal / Fixed locally`. It owns the Review/New Learning information architecture, two-goal hierarchy, session summary labels, Home panel alignment and title-only Instrument Serif contract. Preview deployment and human retest remain pending.
...
- V2 status: Stage 1 through the historical PF-001 V2-8-2.3 closeout are complete within their approved scopes. PF-001 is `High / Closed`; PF-002 is `Normal / Fixed locally`, so V2-8-2.3 is active again. Production V1 and Neon `main` remain Schema Version 5. V2-8-3 Gate 2 is paused until PF-002 closes or is explicitly accepted, and would still require fresh approval. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
...
- V2-8-2.3 uses numbered `PF` records, priority/status rules, bounded local fixes, exact-commit Preview retest and human closure. PF-001 is closed. PF-002 is fixed locally but not deployed or human-retested; Gate 2 remains paused.
