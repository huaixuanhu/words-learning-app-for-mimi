...
- V2-3 locks Schema Version 6 for current local/application snapshots and backup version 3 for JSON backups. Existing migrations must not be rewritten. `0003_v2_schema6_data_model.sql` has now been executed on deleted non-Production proof/rehearsal children and long-lived `staging`; it remains unexecuted on Production `main`.
