...
- Canonical active V2 Stage 8-2.3 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; it owns the pre-launch protected Preview issue register and closure evidence. It was reopened for PF-002 after the historical PF-001 closeout.
...
- Canonical active PF-002 child plan: `plan_docs/PLAN_V2_STAGE8_2_3_2_LEARNING_NAVIGATION_GOAL_HIERARCHY_TYPOGRAPHY.md`; PF-002 is `Normal / Preview-ready`. It owns the Review/New Learning information architecture, two-goal hierarchy, session summary labels, Home panel alignment and font-only rollback. The historical exact protected Preview passed machine acceptance, but the user found Instrument Serif uncomfortable in real use; titles now return to Geist and large vocabulary restores its prior Georgia fallback. The Geist rollback exact protected Preview is Ready; human retest remains pending.
- Canonical PF-003 record lives directly in `plan_docs/PLAN_V2_STAGE8_2_3_PREVIEW_FEEDBACK_STABILISATION.md`; PF-003 is `Normal / Preview-ready`. Active New Learning and Review now reuse the Recognition completion dialog, current `Review complete` preference and local Mimi completion sound across all three Active modes. It has no child plan because it is a bounded UI-only parity fix; exact protected Preview machine checks pass and human retest remains pending.
...
- V2 status: Stage 1 through the historical PF-001 V2-8-2.3 closeout are complete within their approved scopes. PF-001 is `High / Closed`; PF-002 and PF-003 are `Normal / Preview-ready`. V2-8-2.3 remains active pending human retest for both open items. Production V1 and Neon `main` remain Schema Version 5. V2-8-3 Gate 2 is paused until all open PF items close or are explicitly accepted, and would still require fresh approval. Accepted Motion/reduced-motion and Daily Episode/FSRS behavior remain unchanged.
